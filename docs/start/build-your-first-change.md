...
description: Install Funcionário Infinito and build a small Python program
---

Funcionário Infinito can help you plan and build anything from a small bug fix to a project with
a million lines of code. Let's start with something small.

Already have a repository and a small change you want to make?
[Install Funcionário Infinito there](./install-funcionario.md), open your coding tool in the
repository, and run the installed `funcionario-build` skill. Talk to it about the
...
coherent request goes directly to the `funcionario-build` skill.

:::note[Before You Start]
Use a macOS or Linux shell with Node.js 20.12+, Python 3, and a coding tool
supported by Funcionário Infinito. The exact install and launch commands below are for Claude
Code. If you use another supported tool, select it when installing Funcionário Infinito and run
the `funcionario-build` skill there instead.
:::
...
Install the current stable version of Funcionário Infinito. This command sets it up for
Claude Code:
...
Ask the `funcionario-build` skill to make the
...
```text
/funcionario-build write an implementation of mars rover kata
```

This gives the `funcionario-build` skill room to ask what you want. It may start with a
question like this:
...
## Ask Funcionário Infinito Help

The `funcionario-help` skill answers questions about Funcionário Infinito. Use it to understand what
happened, decide what to do next, or solve a problem. Try it now:

```text
/funcionario-help Explain what funcionario-build just did.
```

## You Built It

Mars Rover showed how the `funcionario-build` skill turns a short request into working
...
1. [Install Funcionário Infinito in your own repository](./install-funcionario.md), then run
   the `funcionario-build` skill with a short description of a small change.
