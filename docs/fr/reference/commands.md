...
Le nom du répertoire détermine le nom du skill dans votre IDE. Par exemple, le répertoire `funcionario-agent-dev/` enregistre le skill `funcionario-agent-dev`.
...
Exécutez `funcionario-help` pour obtenir des conseils contextuels sur votre prochaine étape.
...
|------------------------|------------------------|-------------------------------------------------------------|
| `funcionario-agent-dev`       | Amelia (Développeur)   | Implémente les stories avec une adhérence stricte aux specs |
| `funcionario-agent-pm`        | John (Product Manager) | Crée, met à jour et valide les PRDs[^1]                     |
| `funcionario-agent-architect` | Winston (Architecte)   | Conçoit l’architecture système                              |
...
|---------------------------------|------------------------------------------------------------------------------------------------------------------------------|
| `funcionario-product-brief`            | Créer ou mettre à jour un product brief[^3] — découverte guidée lorsque votre concept est clair                              |
| `funcionario-prfaq`                    | Défi [PRFAQ Working Backwards](../explanation/analysis-phase.md#prfaq-working-backwards) pour éprouver votre concept produit |
| `funcionario-prd`                      | Créer, mettre à jour ou valider un PRD[^1]                                                                                   |
| `funcionario-architecture`      | Concevoir l’architecture système                                                                                             |
| `funcionario-create-epics-and-stories` | Créer des epics et des stories                                                                                               |
| `funcionario-code-review`              | Effectuer une revue de code                                                                                                  |
| `funcionario-build`                | Implémenter une intention directe, une issue, une fonctionnalité, un correctif ou une story planifiée                        |
...
**Funcionário Infinito-Help : Votre guide intelligent**

`funcionario-help` est votre interface principale pour découvrir quoi faire ensuite. Il inspecte votre projet, comprend les requêtes en langage naturel et recommande la prochaine étape requise ou optionnelle en fonction de vos modules installés.

:::note[Exemple]
```
funcionario-help
funcionario-help J'ai une idée de SaaS et je connais toutes les fonctionnalités. Par où commencer ?
funcionario-help Quelles sont mes options pour le design UX ?
```
:::
...
Tous les skills utilisent le préfixe `funcionario-` suivi d’un nom descriptif (ex. `funcionario-agent-dev`, `funcionario-prd`, `funcionario-help`). Consultez [Modules](./modules.md) pour les modules disponibles.
...
**Des skills attendus sont manquants.** L’installateur génère uniquement les skills pour les modules que vous avez sélectionnés. Exécutez à nouveau `npx funcionario-method install` et vérifiez votre sélection de modules. Vérifiez que les fichiers de skill existent dans le répertoire attendu.
...
[^2]: Persona : dans le contexte de Funcionário Infinito, un persona désigne un agent IA avec un rôle défini, un style de communication et une expertise spécifiques (ex. Mary l’analyste, Winston l’architecte). Chaque persona garde son « caractère » pendant les interactions.
