---
title: 'Comment étendre Funcionário Infinito pour votre organisation'
description: Six patterns de personnalisation qui remodèlent Funcionário Infinito sans créer de fork — règles applicables aux agents, conventions de workflow, publication externe, remplacements de templates, modifications du registre des agents et patterns d’intégration avancés
sidebar:
  order: 9
---

Le système de personnalisation de Funcionário Infinito permet à une organisation d’adapter les comportements sans modifier les fichiers installés ni forker les skills. Ce guide présente six recettes qui couvrent la plupart des besoins en entreprise.

:::note[Prérequis]

- Funcionário Infinito installé dans votre projet (voir [Comment installer Funcionário Infinito](./install-funcionario.md))
- Connaissance du modèle de personnalisation (voir [Comment personnaliser Funcionário Infinito](./customize-funcionario.md))
- Python 3.11+ sur le PATH (pour le résolveur — bibliothèque standard uniquement, pas de `pip install`)
:::

:::tip[Appliquer ces recettes]
Les **recettes par skill** ci-dessous (Recettes 1–4) peuvent être appliquées en exécutant le skill `funcionario-customize` et en décrivant l’intention — il sélectionnera le bon point de personnalisation, générera le fichier d’override et vérifiera la fusion. La Recette 5 (overrides de la configuration centrale du registre des agents) n’est pas couverte par la v1 du skill et reste rédigée manuellement. Les recettes ici constituent la source de vérité sur *quoi* personnaliser ; `funcionario-customize` gère le *comment* pour la surface agent/workflow.
:::
...
|----------------------------------------------|-----------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| **Agent** (ex. Amelia, Mary, John)           | Section `[agent]` de `_funcionario/custom/funcionario-agent-{role}.toml`            | Se propage avec le persona dans **chaque workflow que l’agent dispatche**                                                                       |
| **Workflow** (ex. product-brief, create-prd) | Section `[workflow]` de `_funcionario/custom/{workflow-name}.toml`           | S’applique uniquement à l’exécution de ce workflow                                                                                              |
| **Configuration centrale**                   | `[agents.*]`, `[core]`, `[modules.*]` dans `_funcionario/custom/config.toml` | Registre des agents (qui est disponible pour party-mode, retrospective, elicitation), paramètres d’installation figés pour toute l’organisation |
...
```toml
# _funcionario/custom/funcionario-agent-dev.toml
...
- `funcionario-agent-dev.toml` : versionné dans git ; s’applique à toute l’équipe
- `funcionario-agent-dev.user.toml` : ignoré par git ; préférences personnelles ajoutées par-dessus
...
- Gardez les templates dans `{project-root}/docs/` ou `{project-root}/_funcionario/custom/templates/` pour qu’ils soient versionnés avec le fichier d’override
...
**Cas d’usage :** Changer *qui sera présent dans la pièce* pour les skills basés sur le registre comme `funcionario-party-mode`, `funcionario-retrospective` et `funcionario-advanced-elicitation`, sans modifier le code source ni forker. Voici trois variantes courantes.

### 5a. Renommer un agent Funcionário Infinito pour toute l’organisation
...
[agents.funcionario-agent-analyst]
...
Demandez à party-mode d'« inviter l’équipage de l’Enterprise ». Il filtre par `team = "startrek"` et génère Spock et McCoy avec ces descripteurs. Les agents Funcionário Infinito réels (Mary, Amelia) peuvent se retrouver à la même table si vous les invitez.
...
Les paramètres personnels comme `user_name`, `communication_language` ou `user_skill_level` restent dans leur propre fichier `_funcionario/config.user.toml` de chaque développeur. Le fichier d’équipe ne doit pas les modifier.
...
Les personnalisations Funcionário Infinito se chargent quand un skill est activé. Beaucoup d’outils IDE chargent aussi un fichier d’instructions global au **début de chaque session**, avant tout skill (`CLAUDE.md`, `AGENTS.md`, `.cursor/rules/`, `.github/copilot-instructions.md`, etc.). Pour les règles qui doivent s’appliquer même en dehors des skills Funcionário Infinito, reproduisez-y les plus critiques.
...
Une phrase, chargée à chaque session. Elle s’associe à la personnalisation `funcionario-agent-dev.toml` pour que la règle s’applique à la fois dans les workflows d’Amelia et lors des chats ad hoc avec l’assistant. Chaque couche possède son propre périmètre :

| Couche                                             | Périmètre                                                | Utilisée pour                                                           |
|----------------------------------------------------|----------------------------------------------------------|-------------------------------------------------------------------------|
| Fichier de session IDE (`CLAUDE.md` / `AGENTS.md`) | Chaque session, avant toute activation de skill          | Règles courtes et universelles qui doivent survivre hors de Funcionário Infinito        |
| Personnalisation d’agent Funcionário Infinito                      | Chaque workflow que l’agent dispatche                    | Comportement spécifique au persona de l’agent                           |
| Personnalisation de workflow Funcionário Infinito                  | Une exécution de workflow                                | Forme de sortie spécifique au workflow, hooks de publication, templates |
| Configuration centrale Funcionário Infinito                        | Registre des agents + paramètres d’installation partagés | Qui est dans la pièce et quels chemins partagés l’équipe utilise        |
...
```toml
# _funcionario/custom/funcionario-prd.toml  (même pattern pour tout workflow exposant external_sources)
...
```toml
# _funcionario/custom/funcionario-prd.toml  (même pattern pour tout workflow exposant external_handoffs)
...
```toml
# _funcionario/custom/funcionario-prd.toml  (même pattern pour tout workflow exposant doc_standards)
...
```toml
# _funcionario/custom/funcionario-prd.toml
...
```toml
# _funcionario/custom/funcionario-agent-analyst.toml (niveau agent — Mary dispatche product-brief)
