---
title: "Comment personnaliser Funcionário Infinito"
...
- Funcionário Infinito installé dans votre projet (voir [Comment installer Funcionário Infinito](./install-funcionario.md))
- Un moyen d’exécuter le script de résolution — Funcionário Infinito adopte `uv` comme standard (`uv run`, qui provisionne Python pour vous) ; un simple `python3` 3.11+ sur votre PATH fonctionne toujours pendant la transition. Le script n’utilise que `tomllib` de la bibliothèque standard, il n’y a donc rien à `pip install`.
...
```text
Priorité 1 (gagne) : _funcionario/custom/{skill-name}.user.toml  (personnel, ignoré par git)
Priorité 2         : _funcionario/custom/{skill-name}.toml       (équipe/org, versionné dans git)
Priorité 3 (base)  : customize.toml du skill              (valeurs par défaut)
```

Le dossier `_funcionario/custom/` est initialement vide. Les fichiers n’apparaissent que lorsqu’un utilisateur commence à personnaliser.
...
```text
.claude/skills/funcionario-agent-pm/customize.toml
```
...
Créez le répertoire `_funcionario/custom/` à la racine de votre projet s’il n’existe pas. Puis créez un fichier portant le même nom que le skill :

```text
_funcionario/custom/
  funcionario-agent-pm.toml        # overrides d'équipe (versionnés dans git)
  funcionario-agent-pm.user.toml   # préférences personnelles (ignoré par git)
```
...
```toml
# _funcionario/custom/funcionario-agent-pm.toml
...
Tous les exemples ci-dessous supposent le schéma d’agent plat de Funcionário Infinito. Les champs se trouvent directement sous `[agent]` — pas de sous-tables `metadata` ou `persona` imbriquées.
...
persistent_facts = [
  ...
  "file:{project-root}/_funcionario/custom/company-glossary.md",
]
...
activation_steps_append = [
  "Lire {project-root}/_funcionario/custom/company-glossary.md s'il existe.",
]
```
...
**Personnalisation du menu (fusion par `code`).** Le menu est un tableau de tables. Chaque élément possède un champ `code` (convention Funcionário Infinito). Le résolveur fusionne donc par code : les codes correspondants remplacent sur place, les nouveaux codes s’ajoutent.
...
prompt = """
Lire {project-root}/_funcionario/custom/compliance-checklist.md
...
**Référencer des fichiers.** Quand le texte d’un champ doit pointer vers un fichier (dans `persistent_facts`, `activation_steps_prepend`/`activation_steps_append`, ou le `prompt` d’un élément de menu), utilisez un chemin complet partant de `{project-root}`. Même si le fichier se trouve à côté de votre override dans `_funcionario/custom/`, écrivez le chemin complet : `{project-root}/_funcionario/custom/info.md`. L’agent résout `{project-root}` à l’exécution.

### 4. Personnel vs Équipe

**Fichier d’équipe** (`funcionario-agent-pm.toml`) : Versionné dans git. Partagé au sein de l’organisation. À utiliser pour les règles de conformité, le persona de l’entreprise, les capacités personnalisées.

**Fichier personnel** (`funcionario-agent-pm.user.toml`) : Automatiquement ignoré par git. À utiliser pour les ajustements de ton, les préférences de workflow personnelles et les faits privés que l’agent doit garder en tête.

```toml
# _funcionario/custom/funcionario-agent-pm.user.toml
...
`--skill` pointe vers le répertoire installé du skill (où se trouve `customize.toml`). Le nom du skill est déduit du basename du répertoire, et le script cherche automatiquement `_funcionario/custom/{skill-name}.toml` et `{skill-name}.user.toml`.
...
uv run {project-root}/_funcionario/scripts/resolve_customization.py \
  --skill /chemin/absolu/vers/funcionario-agent-pm \
  --project-root {project-root} \
  --key agent
...
Les workflows (skills qui pilotent des processus multi-étapes comme `funcionario-product-brief`) partagent le même mécanisme d’override que les agents. Leur surface personnalisable se trouve sous `[workflow]` au lieu de `[agent]` :

```toml
# _funcionario/custom/funcionario-product-brief.toml
...
4. Charger la configuration (`_funcionario/bmm/config.yaml`) et résoudre les variables standard (nom du projet, langues, chemins, date)
...
Le `customize.toml` par skill couvre le **comportement profond** (hooks, menus, persistent_facts, overrides de persona pour un seul agent ou workflow). Une surface séparée couvre l'**état transversal** — les réponses d’installation et le registre des agents que les skills externes comme `funcionario-party-mode`, `funcionario-retrospective` et `funcionario-advanced-elicitation` consomment. Cette surface se trouve dans quatre fichiers TOML à la racine du projet :

```text
_funcionario/config.toml               (géré par l'installateur)  périmètre équipe : réponses d'installation + registre des agents
_funcionario/config.user.toml          (géré par l'installateur)  périmètre utilisateur : user_name, langue, niveau de skill
_funcionario/custom/config.toml        (rédigé manuellement)      overrides d'équipe (versionnés dans git)
_funcionario/custom/config.user.toml   (rédigé manuellement)      overrides personnels (ignoré par git)
```

### Fusion à quatre couches

```text
Priorité 1 (gagne) : _funcionario/custom/config.user.toml
Priorité 2         : _funcionario/custom/config.toml
Priorité 3         : _funcionario/config.user.toml
Priorité 4 (base)  : _funcionario/config.toml
```
...
- Les sections `[core]` et `[modules.<code>]` — réponses d’installation. Le scope `team` figure dans `_funcionario/config.toml` ; le scope `user` figure dans `_funcionario/config.user.toml`.
...
- `_funcionario/config.toml` et `_funcionario/config.user.toml` sont **régénérés à chaque installation** à partir des réponses collectées pendant le processus d’installation. Traitez-les comme des sorties en lecture seule — les modifications directes seront écrasées à la prochaine installation. Pour changer une réponse d’installation de manière durable, relancez l’installateur (il se souvient de vos réponses précédentes comme valeurs par défaut) ou surchargez la valeur dans `_funcionario/custom/config.toml`.
- `_funcionario/custom/config.toml` et `_funcionario/custom/config.user.toml` ne sont **jamais modifiés** par l’installateur. C’est l’espace approprié pour les agents personnalisés, les overrides de descripteur d’agent, les paramètres imposés par l’équipe et toute valeur que vous souhaitez figer indépendamment des réponses d’installation.

### Exemple — Renommer un agent

```toml
# _funcionario/custom/config.toml (versionné dans git, s'applique à tous les développeurs)
...
Le résolveur fusionne par-dessus le `[agents.funcionario-agent-pm]` écrit par l’installateur. `funcionario-party-mode` et tout autre utilisateur du registre récupèrent automatiquement la nouvelle description.

### Exemple — Ajouter un agent fictif

```toml
# _funcionario/custom/config.user.toml (personnel, ignoré par git)
...
```toml
# _funcionario/custom/config.toml
...
| Besoin                                                   | Utiliser                                                                      |
|----------------------------------------------------------|-------------------------------------------------------------------------------|
| Ajouter des appels d’outils MCP à chaque workflow de dev | Par skill : `_funcionario/custom/funcionario-agent-dev.toml` `persistent_facts`             |
| Ajouter un élément de menu à un agent                    | Par skill : `_funcionario/custom/funcionario-agent-{role}.toml` `[[agent.menu]]`            |
| Remplacer le template de sortie d’un workflow            | Par skill : `_funcionario/custom/{workflow}.toml` override scalaire                  |
| Renommer le descripteur public d’un agent                | **Centrale** : `_funcionario/custom/config.toml` `[agents.<code>]`                   |
| Ajouter un agent personnalisé ou fictif au registre      | **Centrale** : `_funcionario/custom/config.*.toml` nouvelle entrée `[agents.<code>]` |
| Figer les paramètres d’installation pour l’équipe        | **Centrale** : `_funcionario/custom/config.toml` `[modules.<code>]` ou `[core]`      |

Utilisez les deux espaces dans le même projet selon vos besoins.

## Exemples concrets

Pour des recettes orientées entreprise (façonner un agent à travers tous les workflows qu’il gère, imposer les conventions d’organisation, publier les livrables vers Confluence et Jira, personnaliser le registre des agents et remplacer vos propres templates de sortie), consultez [Comment étendre Funcionário Infinito pour votre organisation](./expand-funcionario-for-your-org.md).

## Dépannage

**La personnalisation n’apparaît pas ?**

- Vérifiez que votre fichier se trouve dans `_funcionario/custom/` avec le nom de skill correct
...
- Exécutez le skill `funcionario-customize` — il énumère chaque skill personnalisable installé dans votre projet, montre lesquels ont déjà des overrides et vous guide pour en ajouter ou en modifier.
...
- Supprimez votre fichier d’override de `_funcionario/custom/` — le skill revient à ses valeurs par défaut intégrées.
