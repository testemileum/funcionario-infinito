...
description: Choose a generate skill after implementation — simple coverage with funcionario-qa-generate-e2e-tests, or heavier coverage with funcionario-testarch-automate.
sidebar:
  order: 4
---
...
`funcionario-qa-generate-e2e-tests` and `funcionario-testarch-automate` generate tests
...
patterns. See [how the built-in skill runs](#run-funcionario-qa-generate-e2e-tests).
...
| Factor | `funcionario-qa-generate-e2e-tests` | `funcionario-testarch-automate` |
| --- | --- | --- |
...
Most projects should start with `funcionario-qa-generate-e2e-tests`. Use
`funcionario-testarch-automate` when you want the heavier generate of the same
kind of work.
:::
...
## Run `funcionario-qa-generate-e2e-tests`
...
```text
/funcionario-qa-generate-e2e-tests Create API and E2E tests for the login flow.
```
...
`funcionario-qa-generate-e2e-tests` generates tests only. It does not review the
implementation — that is `funcionario-build` during the run, or
[`funcionario-code-review`](review-a-change.md) if you want another pass.
...
Install the TEA module when you want `funcionario-testarch-automate` — heavier
generate of the same kind of work. Automate still generates from existing
code and can run standalone.

`funcionario-testarch-test-design` is optional before Automate. It improves the
run; it is not a prerequisite.

`funcionario-testarch-trace` is optional after generation, to check coverage. It
...
in the [TEA documentation](https://funcionario-infinito.github.io/funcionario-method-test-architecture-enterprise/).
Install it with the rest of Funcionário Infinito; see [Add Modules](../customize/add-modules.md)
for how modules are selected.

## Where It Fits

[`funcionario-build`](build-a-change.md) implements a change and, if a suite
...
epic to finish. A typical sequence is implement with `funcionario-build`,
optionally [walk through the result](walk-through-a-change.md), then generate
coverage here. After a whole epic, `funcionario-retrospective` is a different
