...
description: Use funcionario-build to turn a request, issue, spec, or story into implemented and reviewed code.
sidebar:
  order: 1
---

The core implementation skill is `funcionario-build`. It takes any expression of
...
[how a run works](#run-funcionario-build).

## Size the Work

Use the smallest amount of Funcionário Infinito that safely fits the change. A typical
session is one goal: about 500 lines of code added or changed (not counting
tests) in a small handful of files. If it fits, give it to `funcionario-build`. If it
doesn't, plan that bigger piece of work first — see
[Choose a Planning Path](../plan/choose-a-planning-path.md). You
often cannot tell until you try; if you aren't sure, ask `funcionario-help`.
...
production, `funcionario-build` is likely worth it.

## Run `funcionario-build`
...
```text
/funcionario-build Fix the login validation bug that allows empty passwords.
```

```text
/funcionario-build Fix https://github.com/org/repo/issues/42.
```

```text
/funcionario-build Implement the intent in
_funcionario-output/implementation-artifacts/my-intent.md.
```
...
the exp check entirely. /funcionario-build
```
...
`funcionario-build` starts from your request and investigates the codebase and any
...
After investigation, `funcionario-build` routes to the smallest safe path. It reports
...
After that decision, `funcionario-build` implements the change, reviews its own work
...
When it finishes, `funcionario-build` gives you a short summary and offers the usual
...
last commit cleanly. Then start a fresh chat and run `funcionario-build` again with a
different approach.
:::
...
`funcionario-build` writes them to `deferred-work.md` in your implementation artifacts
...
item into a fresh `funcionario-build` run later.

## When to Plan First

Add a spec, or PRD, UX, architecture, and story planning, before running
`funcionario-build` when:
...
retrospectives cover the combined result. `funcionario-build` handles one unit. It does
not own the backlog, pick the next story, or replace those later checks.

Use `funcionario-build` for foundational, risky, or important stories where your
decisions may set patterns for later work. Once those patterns are stable,
`funcionario-build-auto` can run one unit without waiting for you; see
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `funcionario-build`          | Implement and review one direct intent or planned story with human checkpoints (this page)                                                                    | Implementation record + code                     |
| `funcionario-build-auto`     | Implement and review one unit unattended for a caller or orchestrator ([Autonomous Development Loops](./autonomous-development-loops.md))                     | Implementation record + code + terminal status   |
| `funcionario-code-review`    | Review any code change with several independent reviewers ([Review a Change](./review-a-change.md))                                                           | Findings + applied patches                       |
| `funcionario-correct-course` | Assess the impact of a significant mid-sprint change ([Break Work into Stories and Track It](../plan/break-work-into-stories-and-track-it.md#correct-course)) | Updated plan or re-routing                       |
| `funcionario-retrospective`  | Review a completed epic against the evidence it left behind ([Finish an Epic](./finish-an-epic.md))                                                           | Retro document, action items, acceptance verdict |

Clear one-session work enters `funcionario-build` directly. A spec-backed epic uses
...
selecting each unit. `funcionario-build-auto` does not orchestrate those units: an AI
coding session or another orchestrator, such as funcionario-loop, dispatches one
worker per unit. See
...
You can. The plan-and-implement half of `funcionario-build` usually takes about as
...
`funcionario-build` to take the one-shot route, or to skip review. But if you are
