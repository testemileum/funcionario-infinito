---
title: 'Funcionário Infinito 관련 질문에 답을 얻는 방법'
description: LLM을 사용해 Funcionário Infinito 관련 질문에 빠르게 답하기
sidebar:
  order: 9
---

Funcionário Infinito의 내장 도움말, 소스 문서, 커뮤니티를 사용해 답을 얻으세요. 가장 빠른 방법부터 가장 꼼꼼한 방법까지 순서대로 소개합니다.

## 1. Funcionário Infinito 도움말에게 묻기

답을 얻는 가장 빠른 방법입니다. `funcionario-help` 스킬은 AI 세션에서 바로 사용할 수 있으며 질문의 80% 이상을 처리합니다. 프로젝트를 검사하고 완료한 작업을 확인한 뒤 다음에 무엇을 해야 할지 알려줍니다.

```
funcionario-help SaaS 아이디어가 있고 기능도 모두 알고 있습니다. 어디서 시작하나요?
funcionario-help UX 설계에는 어떤 선택지가 있나요?
funcionario-help PRD 워크플로에서 막혔어요
```

:::tip
플랫폼에 따라 `/funcionario-help` 또는 `$funcionario-help`도 사용할 수 있지만, `funcionario-help`만 입력해도 어디서나 동작합니다.
:::
...
**A:** `funcionario-build`를 실행하세요. 직접 작성한 의도, 이슈, 사양 또는 계획된 스토리를 전달하면 사용 가능한 컨텍스트에 맞춰 필요한 수준으로 요구사항을 명확히 하고 계획, 구현, 리뷰를 진행합니다.
:::
...
AI가 로컬 파일을 읽을 수 없다면(ChatGPT, Claude.ai 등) [Funcionário Infinito 문서 사이트](https://docs.funcionario-infinito.com.br/)를 여세요.

## 3. 사람에게 묻기

Funcionário Infinito 도움말이나 소스로도 답을 얻지 못했다면, 이제 훨씬 나은 질문을 할 수 있습니다.
