---
title: '조직을 위해 Funcionário Infinito 확장하기'
description: 포크 없이 Funcionário Infinito를 재구성하는 여섯 가지 커스터마이징 패턴 - 에이전트 전반 규칙, 워크플로 관례, 외부 게시, 템플릿 교체, 에이전트 명단 변경, 고급 통합 패턴
sidebar:
  order: 7
---

Funcionário Infinito의 커스터마이징 영역을 사용하면 설치된 파일을 수정하거나 스킬을 포크하지 않고도 조직에 맞게 동작을 바꿀 수 있습니다. 이 가이드는 대부분의 엔터프라이즈 요구를 다루는 여섯 가지 레시피를 소개합니다.

:::note[필수 조건]

- 프로젝트에 Funcionário Infinito 설치([Funcionário Infinito 설치 방법](../start/install-funcionario.md) 참고)
- 커스터마이징 모델 이해([Funcionário Infinito 커스터마이징 방법](./customize-funcionario.md) 참고)
- PATH의 Python 3.11+(병합 스크립트용, stdlib만 사용하며 `pip install` 불필요)
:::

:::tip[이 레시피 적용하기]
아래 **스킬별 레시피**(레시피 1-4)는 `funcionario-customize` 스킬을 실행하고 의도를 설명해 적용할 수 있습니다. 스킬이 알맞은 영역을 고르고 오버라이드 파일을 작성한 뒤 병합 결과를 검증합니다. 레시피 5처럼 에이전트 명단을 바꾸는 중앙 설정 오버라이드는 v1 스킬 범위 밖이므로 직접 작성해야 합니다. 이 문서의 레시피는 *무엇을* 오버라이드할지 정하는 기준을 제공합니다. `funcionario-customize`는 에이전트와 워크플로 영역에서 이를 *어떻게* 적용할지 안내합니다.
:::
...
| 계층 | 오버라이드 위치 | 범위 |
| --- | --- | --- |
| **에이전트**(예: Amelia, Mary, John) | `_funcionario/custom/funcionario-agent-{role}.toml`의 `[agent]` 섹션 | 에이전트가 실행하는 **모든 워크플로**에 페르소나와 함께 이동 |
| **워크플로**(예: 제품 개요, PRD 생성) | `_funcionario/custom/{workflow-name}.toml`의 `[workflow]` 섹션 | 해당 워크플로 실행에만 적용 |
| **중앙 설정** | `_funcionario/custom/config.toml`의 `[agents.*]`, `[core]`, `[modules.*]` | 에이전트 명단(파티 모드, 회고, 도출에서 누가 가능한지), 조직 전체로 고정된 설치 시 설정 |
...
- `funcionario-agent-dev.toml`: git에 커밋하고 팀 전체에 적용합니다
- `funcionario-agent-dev.user.toml`: git에서 무시되며 개인 선호를 위에 덧씌웁니다
...
- 템플릿은 `{project-root}/docs/` 또는 `{project-root}/_funcionario/custom/templates/`에 두어 오버라이드 파일과 함께 버전 관리합니다
...
**사용 사례:** 소스를 수정하거나 포크하지 않고 `funcionario-party-mode`, `funcionario-retrospective`, `funcionario-advanced-elicitation` 같은 명단 기반 스킬에서 *방에 누가 있는지* 바꿉니다. 세 가지 흔한 변형은 다음과 같습니다.

### 5a. Funcionário Infinito 에이전트를 조직 전체에서 리브랜딩

실제 에이전트마다 설치 프로그램이 `module.yaml`에서 합성한 설명자가 있습니다. 이를 오버라이드하면 명단을 사용하는 모든 스킬에서 목소리와 표현 방식을 바꿀 수 있습니다.

```toml
# _funcionario/custom/config.toml (커밋됨 - 모든 개발자에게 적용)
...
파티 모드에 "엔터프라이즈 승무원을 초대해 줘"라고 요청하면 `team = "startrek"`으로 필터링하고 스팍과 맥코이를 생성합니다. 요청하면 실제 Funcionário Infinito 에이전트(Mary, Amelia)도 같은 테이블에 앉을 수 있습니다.
...
`user_name`, `communication_language`, `user_skill_level` 같은 개인 설정은 각 개발자의 `_funcionario/config.user.toml` 아래에 둡니다. 팀 파일은 이를 건드리지 않는 것이 좋습니다.
...
Funcionário Infinito 커스터마이징은 스킬이 활성화될 때 로드됩니다. 많은 IDE 도구는 스킬이 실행되기 전 **모든 세션 시작 시** 전역 지침 파일도 로드합니다(`CLAUDE.md`, `AGENTS.md`, `.cursor/rules/`, `.github/copilot-instructions.md` 등). Funcionário Infinito 스킬 밖에서도 지켜져야 하는 규칙은 거기에도 핵심만 반복하세요.
...
한 문장이 매 세션에 로드됩니다. `funcionario-agent-dev.toml` 커스터마이징과 짝을 이뤄 Amelia의 워크플로 안과 어시스턴트와의 일반 채팅 모두에 규칙을 적용합니다.

| 계층 | 범위 | 사용처 |
| --- | --- | --- |
| IDE 세션 파일(`CLAUDE.md` / `AGENTS.md`) | 모든 세션, 스킬 활성화 전 | Funcionário Infinito 밖에서도 적용해야 하는 짧은 보편 규칙 |
| Funcionário Infinito 에이전트 커스터마이징 | 에이전트가 실행하는 모든 워크플로 | 에이전트 페르소나별 동작 |
| Funcionário Infinito 워크플로 커스터마이징 | 하나의 워크플로 실행 | 워크플로별 출력 형태, 게시 후크, 템플릿 |
| Funcionário Infinito 중앙 설정 | 에이전트 명단 + 공유 설치 설정 | 방에 누가 있고 팀이 어떤 공유 경로를 쓰는지 |

IDE 파일은 **간결하게** 유지하세요. 잘 고른 열두 줄이 긴 목록보다 효과적입니다. 모델은 이를 매 턴 읽으므로 불필요한 내용이 많으면 중요한 지침이 묻힙니다.

## 레시피 6: 고급 통합 패턴

몇몇 Funcionário Infinito 워크플로는 레시피 1-5에서 다룬 기본보다 더 넓은 설정 영역을 제공합니다. 필요할 때 불러오는 지식 소스, 자동 출력 게시, 완료 시점 문서 표준, 교체 가능한 템플릿 같은 패턴이 여러 워크플로에 나타납니다. 어떤 필드를 제공하는지는 워크플로의 `customize.toml`을 확인하세요. 아래 예시는 모든 필드를 제공하는 `funcionario-prd`를 사용하지만, 같은 패턴은 해당 필드가 있는 모든 워크플로에 적용됩니다.
...
```toml
# _funcionario/custom/funcionario-prd.toml  (external_sources를 노출하는 모든 워크플로에서 같은 패턴 사용)
...
```toml
# _funcionario/custom/funcionario-prd.toml  (external_handoffs를 노출하는 모든 워크플로에서 같은 패턴 사용)
...
```toml
# _funcionario/custom/funcionario-prd.toml  (doc_standards를 노출하는 모든 워크플로에서 같은 패턴 사용)
...
에이전트는 템플릿이 정의한 구조에 적응합니다. 템플릿은 `{project-root}/docs/` 또는 `{project-root}/_funcionario/custom/templates/` 아래에 두어 오버라이드 파일과 함께 버전 관리하세요. 다중 조직 저장소에서는 `.user.toml`로 개별 팀이 커밋된 팀 파일을 건드리지 않고 자체 템플릿을 가리키게 할 수 있습니다.

## 레시피 조합

여섯 레시피는 모두 조합할 수 있습니다. 현실적인 엔터프라이즈용 `funcionario-product-brief` 오버라이드는 한 파일에서 `persistent_facts`(레시피 2), `on_complete`(레시피 3), `brief_template`(레시피 4)을 설정할 수 있습니다. 에이전트 수준 규칙(레시피 1)은 에이전트 이름의 별도 파일에 있고, 중앙 설정(레시피 5)은 공유 명단과 팀 설정을 고정하며, 고급 통합 패턴(레시피 6)은 외부 소스와 전달 작업을 설정합니다. 모든 계층은 함께 적용됩니다.

```toml
# _funcionario/custom/funcionario-product-brief.toml (워크플로 수준)
...
```toml
# _funcionario/custom/funcionario-agent-analyst.toml (에이전트 수준 - Mary가 product-brief를 실행)

[agent]
persistent_facts = ["도메인이 헬스케어, 금융, 아동 데이터와 관련되면 항상 '규제 검토' 섹션을 포함하세요."]
```

결과: Mary는 페르소나 활성화에서 규제 리뷰 규칙을 로드합니다. 사용자가 제품 개요 메뉴 항목을 선택하면 워크플로는 자체 관례를 그 위에 로드하고 엔터프라이즈 템플릿에 작성한 뒤 완료 시 Confluence에 게시합니다. 모든 계층이 함께 작동하며 Funcionário Infinito 소스는 수정하지 않습니다.

## 문제 해결

**오버라이드가 적용되지 않나요?** 파일이 `_funcionario/custom/` 아래 정확한 스킬 디렉터리 이름으로 있는지 확인하세요(예: `funcionario-agent-dev.toml`, `funcionario-dev.toml` 아님). [Funcionário Infinito 커스터마이징 방법의 문제 해결](./customize-funcionario.md#문제-해결)을 참고하세요.
