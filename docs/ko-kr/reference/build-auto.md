---
title: 자율 개발 루프
description: funcionario-build-auto를 단일 작업자로 사용해 Build 구현 모델을 자동화하는 방법
sidebar:
  order: 6
---

`funcionario-build-auto`는 표준 [변경 사항 구현하기](../build/build-a-change.md) 모델에서 세션 하나에 들어오는 작업 단위 하나를 사람의 개입 없이 처리합니다. 호출 한 번으로 하나의 의도나 스토리를 명확히 하고 계획·구현·검토한 뒤, 사람이나 오케스트레이터가 후속 처리할 수 있는 종료 상태를 남깁니다.

Build Auto는 다음 스토리를 선택하거나 백로그 전체를 반복 실행하거나 에픽을 조율하거나 회고를 실행하지 않습니다. 자신이 실행하는 구현 작업과 새로 만들거나 이어서 쓰는 기록만 담당합니다. 백로그 정책과 작업 배정은 사람이나 AI 코딩 세션, funcionario-loop 같은 오케스트레이터가 맡습니다.

## 하는 일

`funcionario-build-auto`는 사람 개입 없는 구현 작업을 한 번 실행합니다.
...
기본 입력은 호출 프롬프트입니다. `funcionario-build-auto`는 이 프롬프트를 완성된 구현 계획이 아닌 워크플로 입력으로 다룹니다.
...
### funcionario-loop로 순서가 지정된 목록 실행

선택 사항인 [funcionario-loop](https://github.com/funcionario-infinito/funcionario-loop) 오케스트레이터는 사양 폴더의 `stories.yaml`을 목록 순서대로 처리합니다. 의존성 그래프를 추론하지 않는 선형 스케줄러이므로 각 스토리의 선행 작업이 앞에 오도록 목록을 정렬해야 합니다.

스토리 하나를 선택하면 해당 스토리만 실행합니다. "여기서 시작해 나머지를 모두 실행"한다는 뜻이 아닙니다. 회고는 에픽을 마무리하는 별도 작업입니다. funcionario-loop가 실행을 권할 수는 있지만 실제 회고는 `funcionario-retrospective`가 수행합니다.
...
프로젝트 수준의 병렬 실행에는 더 높은 조율 계층이나 에픽별 담당자가 필요합니다. 의존성과 통합 경계가 명확하다면 독립된 에픽 흐름을 병렬로 실행할 수 있습니다. funcionario-loop의 순차 스토리 스케줄러는 이러한 프로젝트 수준 조율을 제공하지 않습니다.

## 컨텍스트 입력

활성화되면 워크플로는 다음 항목을 확인합니다.

- `_funcionario/config.toml`, `_funcionario/config.user.toml`, `_funcionario/custom/` 아래의 선택적 팀/사용자 오버라이드
...
`funcionario-build-auto`를 통합하는 오케스트레이터는 다음을 해야 합니다.
...
차단 원인을 해결한 뒤에는 보통 `funcionario-build-auto`를 새로 실행해야 합니다. 기존 작업을 재사용하려면 자동 탐색에 기대지 말고 검증된 사양 경로를 명시적으로 전달하세요.
