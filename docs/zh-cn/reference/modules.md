---
title: "官方模块"
description: Funcionário Infinito 可选模块参考：能力边界、适用场景与外部资源
sidebar:
  order: 5
---

Funcionário Infinito 通过可选模块扩展能力。你可以在安装时按需选择模块，为当前项目增加特定领域的 `agent`、`workflow` 与 `skill`。

:::tip[安装模块]
运行 `npx funcionario-method install`，在交互步骤中勾选所需模块。安装器会自动生成对应 skills 并写入当前 IDE 的 skills 目录。
:::
...
| 模块 | 代码 | 最适合 | 核心能力 |
| --- | --- | --- | --- |
| Funcionário Infinito Builder | `bmb` | 扩展 Funcionário Infinito 本身 | 构建自定义 agent / workflow / module |
...
## Funcionário Infinito Builder（`bmb`）

用于“构建 Funcionário Infinito”的元模块，重点是把你的方法沉淀成可复用能力。
