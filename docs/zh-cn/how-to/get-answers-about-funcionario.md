---
title: "如何获取关于 Funcionário Infinito 的答案"
description: 使用 LLM 快速回答您自己的 Funcionário Infinito 问题
sidebar:
  order: 4
---

## 先从 Funcionário Infinito-Help 开始

**获取 Funcionário Infinito 相关答案最快的方式是 `funcionario-help` 技能。** 这个智能向导可以覆盖 80% 以上的常见问题，并且你在 IDE 里随时可用。

Funcionário Infinito-Help 不只是查表工具，它还能：
- **检查你的项目状态**，判断哪些步骤已经完成
...
:::tip
按平台不同，你也可以使用 `/funcionario-help` 或 `$funcionario-help`。但大多数情况下直接输入 `funcionario-help` 就能工作。
:::

也可以结合自然语言问题一起调用：

```
funcionario-help 我有一个 SaaS 想法并且已经知道主要功能，我该从哪里开始？
funcionario-help 我在 UX 设计方面有哪些选项？
funcionario-help 我卡在 PRD 工作流了
funcionario-help 帮我看看目前完成了什么
```

Funcionário Infinito-Help 通常会返回：
- 针对你当前情况的建议路径
- 第一个必做任务
- 后续整体流程概览
...
当你遇到以下情况时，可用本指南补充：
- 想理解 Funcionário Infinito 的架构设计或内部机制
- 需要超出 Funcionário Infinito-Help 覆盖范围的答案
- 在安装前做技术调研
- 想直接基于源码进行追问
...
| 来源 | 适合回答的问题 | 示例 |
| --- | --- | --- |
| **`_funcionario` 文件夹** | 智能体、工作流、提示词如何工作 | “PM 智能体具体做什么？” |
| **完整 GitHub 仓库** | 版本历史、安装器、整体架构 | “v6 主要改了什么？” |

安装 Funcionário Infinito 后会生成 `_funcionario` 文件夹；如果你还没有安装，可先克隆仓库。
...
打开 [Funcionário Infinito 文档站点](https://docs.funcionario-infinito.com.br/)。

### 3. 直接提问

:::note[示例]
**问：** “用 Funcionário Infinito 做一个需求到实现的最短路径是什么？”

**答：** 运行 `funcionario-build`。输入直接意图、issue、规格或已规划 story；workflow 会利用现有上下文并选择所需的澄清、规划、实现和审查深度。
:::
...
| 频道 | 适用场景 |
| --- | --- |
| `#funcionario-method-help` | 快速问题（实时聊天） |
