...
Každá instalace Funcionário Infinito zahrnuje **základní modul** — malou sadu skills, které fungují napříč všemi projekty, všemi moduly a všemi fázemi. Tato stránka pokrývá těchto sedm základních skills: čtyři jádrové nástroje plus tři **myšlenkové skills** (brainstorming, forge idea, party mode).

:::tip[Rychlá cesta]
Spusťte jakýkoli nástroj zadáním jeho názvu skillu (např. `funcionario-help`) ve vašem IDE. Nevyžaduje relaci agenta.
:::
...
| Nástroj                                                   | Účel                                                                                                                      |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| [`funcionario-help`](#funcionario-help)                                 | Kontextové poradenství, co dělat dál                                                                                      |
| [`funcionario-advanced-elicitation`](#funcionario-advanced-elicitation) | Iterativní zdokonalování LLM výstupu                                                                                      |
| [`funcionario-review`](#funcionario-review)                             | Revize z více perspektiv — adversariální, hraniční případy a mezery ve verifikaci pro kód; struktura a text pro dokumenty |
| [`funcionario-customize`](#funcionario-customize)                       | Vytváření a ověřování přizpůsobení Funcionário Infinito                                                                                   |

**Myšlenkové skills:**

| Nástroj                                     | Účel                                                                   |
| ------------------------------------------- | ---------------------------------------------------------------------- |
| [`funcionario-brainstorming`](#funcionario-brainstorming) | Facilitace interaktivních brainstormingových sezení                    |
| [`funcionario-forge-idea`](#funcionario-forge-idea)       | Zátěžový test nápadu, dokud se nezpevní, nepotvrdí, nebo levně nezemře |
| [`funcionario-party-mode`](#funcionario-party-mode)       | Orchestrace skupinových diskuzí více agentů                            |

:::note[Přesunuto a odstraněno]
`funcionario-spec` se nyní dodává s modulem BMM jako plánovací workflow Fáze 2 — viz [Mapa workflow](./workflow-map.md). Utility `funcionario-shard-doc` a `funcionario-index-docs` byly odstraněny. Dřívější skills `funcionario-editorial-review`, `funcionario-editorial-review-prose`, `funcionario-editorial-review-structure`, `funcionario-review-adversarial-general`, `funcionario-review-edge-case-hunter` a `funcionario-review-verification-gap` jsou všechny sloučeny do `funcionario-review`, jehož redakční perspektivy nahrazují samostatný redakční skill; staré identifikátory se stále rozliší přes přesměrování kvůli kompatibilitě.
:::
...
- Jste noví v Funcionário Infinito a potřebujete orientaci
...
**Vstup:** Volitelný dotaz v přirozeném jazyce (např. `funcionario-help I have a SaaS idea, where do I start?`)

**Výstup:** Prioritizovaný seznam doporučených dalších kroků s příkazy skills

## funcionario-advanced-elicitation

**Přiměje LLM přehodnotit, zdokonalit a vylepšit svůj nedávný výstup.** — Sdílený zdokonalovací checkpoint Funcionário Infinito: ostatní skills jej vyvolávají při přirozených pauzách a vy jej můžete zavolat přímo na cokoli nedávného v konverzaci.
...
**Vytváření a ověřování přizpůsobení.** — Pomůže vám změnit chování nainstalovaného Funcionário Infinito agenta nebo workflow bez ručního psaní TOML.
...
1. Skenuje nainstalované Funcionário Infinito skills pro přizpůsobitelné plochy
2. Vybere správný rozsah pro požadovanou změnu
3. Zapíše přepisové soubory pod `_funcionario/custom/`
4. Ověří sloučenou konfiguraci

**Vstup:** Popis požadovaného přizpůsobení v přirozeném jazyce

**Výstup:** TOML přepisové soubory pod `_funcionario/custom/`. Podrobný návod viz [Jak přizpůsobit Funcionário Infinito](../how-to/customize-funcionario.md)
...
**Orchestrace skupinových diskuzí více agentů.** — Načte všechny nainstalované Funcionário Infinito agenty a facilituje přirozenou konverzaci, kde každý agent přispívá svou unikátní odborností a osobností.
