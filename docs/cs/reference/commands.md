---
title: Skills
description: Reference Funcionário Infinito skills — co to je, jak fungují a kde je najít.
sidebar:
  order: 4
---

Skills jsou předpřipravené prompty, které načítají agenty, spouštějí workflow nebo provádějí úkoly ve vašem IDE. Instalátor Funcionário Infinito je generuje z vašich nainstalovaných modulů při instalaci. Pokud později přidáte, odeberete nebo změníte moduly, přeinstalujte pro synchronizaci skills (viz [Řešení problémů](#řešení-problémů)).

## Skills vs. spouštěče nabídky agentů

Funcionário Infinito nabízí dva způsoby zahájení práce a slouží k různým účelům.

| Mechanismus | Jak se vyvolává | Co se stane |
| --- | --- | --- |
| **Skill** | Zadejte název skillu (např. `funcionario-help`) ve vašem IDE | Přímo načte agenta, spustí workflow nebo provede úkol |
...
Když spustíte `npx funcionario-method install`, instalátor čte manifesty každého vybraného modulu a zapíše jeden skill na agenta, workflow, úkol a nástroj. Každý skill je adresář obsahující soubor `SKILL.md`, který instruuje AI k načtení odpovídajícího zdrojového souboru a následování jeho instrukcí.
...
Název adresáře určuje název skillu ve vašem IDE. Například adresář `funcionario-agent-dev/` registruje skill `funcionario-agent-dev`.
...
Spusťte `funcionario-help` pro kontextové poradenství k dalšímu kroku.
...
| --- | --- | --- |
| `funcionario-agent-dev` | Amelia (Developer) | Implementuje stories s přísným dodržováním specifikací |
| `funcionario-pm` | John (Product Manager) | Vytváří a validuje PRD |
| `funcionario-architect` | Winston (Architect) | Navrhuje systémovou architekturu |
...
| Příklad skillu | Účel |
| --- | --- |
| `funcionario-product-brief` | Vytvoření product briefu — řízené discovery, když je váš koncept jasný |
| `funcionario-prfaq` | [Working Backwards PRFAQ](../explanation/analysis-phase.md#prfaq-working-backwards) výzva pro zátěžový test vašeho produktového konceptu |
| `funcionario-prd` | Vytvoření dokumentu požadavků (PRD) |
| `funcionario-architecture` | Návrh systémové architektury |
| `funcionario-create-epics-and-stories` | Vytvoření epiců a stories |
| `funcionario-code-review` | Spuštění revize kódu |
| `funcionario-build` | Implementace přímého záměru, issue, funkce, opravy nebo naplánované story |
...
**Funcionário Infinito-Help: Váš inteligentní průvodce**

`funcionario-help` je vaše primární rozhraní pro objevení, co dělat dál. Zkoumá váš projekt, rozumí dotazům v přirozeném jazyce a doporučuje další povinný nebo volitelný krok na základě nainstalovaných modulů.
...
Všechny skills používají prefix `funcionario-` následovaný popisným názvem (např. `funcionario-dev`, `funcionario-prd`, `funcionario-help`). Viz [Moduly](./modules.md) pro dostupné moduly.
...
**Očekávané skills chybí.** Instalátor generuje skills pouze pro moduly, které jste vybrali. Spusťte `npx funcionario-method install` znovu a ověřte výběr modulů. Zkontrolujte, že soubory skills existují v očekávaném adresáři.
