...
Funcionário Infinito poskytuje dvě testovací cesty: vestavěného QA agenta pro rychlé generování testů a instalovatelný modul Test Architect pro podnikovou testovací strategii.
...
| **Nastavení** | Nic k instalaci — součástí BMM | Instalace zvlášť přes `npx funcionario-method install` |
...
**Spouštěč:** `QA` nebo `funcionario-qa-generate-e2e-tests`
...
- **Dokumentace:** [Dokumentace modulu TEA](https://funcionario-infinito.github.io/funcionario-method-test-architecture-enterprise/)
- **Instalace:** `npx funcionario-method install` a výběr modulu TEA
...
Quinn workflow Automate se objevuje ve Fázi 4 (Implementace) mapy workflow Funcionário Infinito. Je navržen ke spuštění **po dokončení celého epicu** — jakmile jsou všechny stories v epicu implementovány a zrevidovány. Typická sekvence:

1. Pro každou story v epicu: implementace pomocí Build (`BD` / `funcionario-build`), pak podle potřeby Code Review (`CR`)
2. Po dokončení epicu: generování testů s Quinnem (`QA`) nebo TEA workflow Automate
3. Spuštění retrospektivy (`funcionario-retrospective`) pro zachycení získaných zkušeností
