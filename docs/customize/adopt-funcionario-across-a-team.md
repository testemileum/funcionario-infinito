---
title: 'Adopt Funcionário Infinito Across a Team'
description: Recipes that make every developer's Funcionário Infinito follow your organization's rules, tools, templates, and agent roster — without forking a skill.
sidebar:
  order: 2
---

You lead a team and want every developer's Funcionário Infinito to use the same tools,
follow the same conventions, publish to the same systems, and know the same
people. Each recipe below is one override file under `_funcionario/custom/`,
...
[Customize Funcionário Infinito](./customize-funcionario.md). This page never repeats those
mechanics; it shows what to write.

:::tip[Applying these recipes]
Run the `funcionario-customize` skill and describe the intent; it writes the
...
Every per-skill recipe has a team form and a personal form. `funcionario-agent-dev.toml`
is committed to git and applies to the whole team; `funcionario-agent-dev.user.toml`
...
**File and key:** `_funcionario/custom/funcionario-agent-dev.toml`, `[agent] persistent_facts`.
...
**File and key:** `_funcionario/custom/funcionario-product-brief.toml`, `[workflow] persistent_facts`.
...
**File and key:** `_funcionario/custom/funcionario-product-brief.toml`, `[workflow] on_complete`.
...
**File and key:** `_funcionario/custom/funcionario-product-brief.toml`, `[workflow] brief_template`.
...
`{project-root}/_funcionario/custom/templates/` so they version alongside the
...
**File:** `_funcionario/custom/config.toml` (team) or `_funcionario/custom/config.user.toml` (personal).

Use central config to change who roster-driven skills (`funcionario-party-mode`,
`funcionario-retrospective`, `funcionario-advanced-elicitation`) see, and to pin install
...
[Central configuration](./customize-funcionario.md#central-configuration) for the
file layout.

### 5a. Rebrand an agent for the whole team

**Key:** `[agents.funcionario-agent-analyst] description`.

```toml
# _funcionario/custom/config.toml (committed — applies to every developer)
...
```toml
# _funcionario/custom/config.user.toml (personal — gitignored)
...
Funcionário Infinito agents in the same party.
...
`user_skill_level` stay in each developer's own `_funcionario/config.user.toml`;
the team file should not set them.

## Reinforce global rules in your IDE's session file

Funcionário Infinito customizations load when a skill activates. Most IDE tools also load
...
| Layer | Scope | Use for |
|---|---|---|
| IDE session file (`CLAUDE.md` / `AGENTS.md`) | Every session, before any skill activates | Short, universal rules that should survive outside Funcionário Infinito |
| Funcionário Infinito agent customization | Every workflow the agent dispatches | Agent-specific behavior |
| Funcionário Infinito workflow customization | One workflow run | Output shape, publishing hooks, templates |
| Funcionário Infinito central config | Agent roster and shared install settings | Who is on the roster and which paths the team shares |

Keep the IDE file short. The model reads it every turn.

## Recipe 6: Advanced fields

**File and keys:** `_funcionario/custom/funcionario-prd.toml`, `[workflow] external_sources`,
...
`funcionario-prd`, which has all of them. The same pattern applies wherever a
field appears.
...
(`skill:funcionario-review lenses=structure,prose`). Put broad structural passes
...
```toml
# _funcionario/custom/funcionario-product-brief.toml (workflow)
...
```toml
# _funcionario/custom/funcionario-agent-analyst.toml (agent — Mary dispatches product-brief)
...
`_funcionario/custom/` with the exact skill directory name (`funcionario-agent-dev.toml`,
not `funcionario-dev.toml`). See
[Customize Funcionário Infinito](./customize-funcionario.md#troubleshooting) for the rest.
