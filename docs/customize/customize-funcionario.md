---
title: 'Customize Funcionário Infinito'
description: Change how an installed agent or workflow behaves — with funcionario-customize or by hand — and know what is customizable, where an override lands, and how it merges.
sidebar:
  order: 1
---
...
Funcionário Infinito's installed defaults. Updates do not touch your files, and you do not
edit installed files.

## Start with the guided path

Run the `funcionario-customize` skill and say what you want changed. It scans your
...
| Surface | File | Shapes |
|---|---|---|
| Per-skill override | `_funcionario/custom/<skill>.toml` | How one agent or workflow behaves when it activates: persona, facts, hooks, menu, workflow fields |
| Central configuration | `_funcionario/custom/config.toml` | Install answers and the agent roster that other skills read |

`funcionario-customize` writes per-skill overrides only. Central configuration is
...
[Adopt Funcionário Infinito Across a Team](./adopt-funcionario-across-a-team.md).
...
Central configuration controls how `funcionario-party-mode`, `funcionario-retrospective`,
and `funcionario-advanced-elicitation` introduce the agent. Rewriting Mary's
principles is per-skill; changing the one-line description a party uses
to introduce her is central.

:::note[Prerequisites]

- Funcionário Infinito installed in your project (see [Install Funcionário Infinito](../start/install-funcionario.md)).
- [`uv`](https://docs.astral.sh/uv/) on your PATH. Funcionário Infinito runs the resolver with `uv run`, which provisions Python for you; there is nothing to `pip install`.
:::
...
```text
Priority 1 (wins): _funcionario/custom/<skill>.user.toml   (personal, gitignored)
Priority 2:        _funcionario/custom/<skill>.toml        (team, committed)
Priority 3 (base): the skill's own customize.toml   (shipped defaults)
```

`_funcionario/custom/` starts empty. Files appear only when someone customizes.
...
**Scalars.** Create `_funcionario/custom/` in your project root if it does not
...
activation_steps_append = [
  "Read {project-root}/_funcionario/custom/company-glossary.md if it exists.",
]
```
...
prompt = """
Read {project-root}/_funcionario/custom/compliance-checklist.md
...
`_funcionario/custom/`. The agent resolves `{project-root}` at runtime.

**Team or personal.** The team file (`funcionario-agent-pm.toml`) is committed
and shared: compliance rules, company persona, custom menu items. The
personal file (`funcionario-agent-pm.user.toml`) is gitignored: tone, private
...
`funcionario-product-brief` — use the same files and rules. Their surface lives
...
and each follows the shape rules above. For example, `funcionario-code-review`
exposes `review`, the default review depth:

```toml
# _funcionario/custom/funcionario-code-review.toml
[workflow]
review = "quick"
```
...
```bash
uv run /abs/project/_funcionario/scripts/render_skill.py \
  --project-root /abs/project \
  --skill /abs/path/to/funcionario-build \
...
```text
_funcionario/config.toml               (installer-owned)  team scope: install answers + agent roster
_funcionario/config.user.toml          (installer-owned)  user scope: user_name, language, skill level
_funcionario/custom/config.toml        (human-authored)   team overrides (committed)
_funcionario/custom/config.user.toml   (human-authored)   personal overrides (gitignored)
```

**Four layers**, merged with the same shape rules:

```text
Priority 1 (wins): _funcionario/custom/config.user.toml
Priority 2:        _funcionario/custom/config.toml
Priority 3:        _funcionario/config.user.toml
Priority 4 (base): _funcionario/config.toml
```

**What lives where.** The installer splits its answers by the `scope:`
declared on each prompt in a module's `module.yaml`: `[core]` and
`[modules.<code>]` answers with scope `team` land in `_funcionario/config.toml`,
scope `user` in `_funcionario/config.user.toml`. `[agents.<code>]` holds each
...
override the value in `_funcionario/custom/config.toml`. The two `_funcionario/custom/`
...
```toml
# _funcionario/custom/config.user.toml
...
| Need | Use |
|---|---|
| Add MCP tool calls to every dev workflow | Per-skill: `_funcionario/custom/funcionario-agent-dev.toml` `persistent_facts` |
| Add a menu item to an agent | Per-skill: `_funcionario/custom/funcionario-agent-<role>.toml` `[[agent.menu]]` |
| Swap a workflow's output template | Per-skill: `_funcionario/custom/<workflow>.toml` scalar override |
| Rebrand an agent's public descriptor | Central: `_funcionario/custom/config.toml` `[agents.<code>]` |
| Add a custom or fictional agent to the roster | Central: `_funcionario/custom/config.*.toml` new `[agents.<code>]` |
| Pin team-enforced install settings | Central: `_funcionario/custom/config.toml` `[modules.<code>]` or `[core]` |
...
the skill name from that folder and finds the matching `_funcionario/custom/`
files itself. Output is always JSON.

`--project-root` names the project whose `_funcionario/custom/` files apply.
...
skill is installed under your home directory and a `~/_funcionario` exists there
...
**Customization not appearing.** Check that the file is in `_funcionario/custom/`
...
**See what is customizable.** Run `funcionario-customize`, which lists every
customizable skill and which already have overrides, or read the skill's
`customize.toml` directly.

**Reset.** Delete the override file from `_funcionario/custom/`. The skill falls
