---
title: 'Cách mở rộng Funcionário Infinito cho tổ chức của bạn'
description: Năm mẫu tùy chỉnh giúp thay đổi Funcionário Infinito mà không cần fork, gồm quy tắc ở cấp agent, quy ước workflow, xuất bản ra hệ thống ngoài, thay template và điều chỉnh danh sách agent
sidebar:
  order: 9
---

Bề mặt tùy chỉnh của Funcionário Infinito cho phép một tổ chức định hình lại hành vi mà không phải sửa file đã cài hay fork skill. Hướng dẫn này trình bày năm công thức mẫu (recipe) bao phủ phần lớn nhu cầu ở môi trường doanh nghiệp.

:::note[Điều kiện tiên quyết]

- Funcionário Infinito đã được cài trong dự án của bạn (xem [Cách cài đặt Funcionário Infinito](./install-funcionario.md))
- Đã quen với mô hình tùy chỉnh (xem [Cách tùy chỉnh Funcionário Infinito](./customize-funcionario.md))
- Python 3.11+ có trên PATH để chạy resolver, chỉ dùng stdlib, không cần `pip install`
:::

:::tip[Cách áp dụng các công thức mẫu này]
Những **công thức mẫu theo từng skill** bên dưới, tức Recipe 1 đến Recipe 4, có thể được áp dụng bằng cách chạy skill `funcionario-customize` rồi mô tả ý định. Skill này sẽ tự chọn đúng bề mặt, viết file override và xác minh kết quả merge. Riêng Recipe 5, tức override cấu hình trung tâm để chỉnh danh sách agent (agent roster), hiện chưa nằm trong phạm vi v1 của skill nên vẫn cần viết tay. Các recipe trong trang này là nguồn sự thật cho phần *nên override cái gì*; `funcionario-customize` phụ trách phần *thực hiện ra sao* ở lớp agent/workflow.
:::
...
|---|---|---|
| **Agent** như Amelia, Mary, John | section `[agent]` trong `_funcionario/custom/funcionario-agent-{role}.toml` | Đi cùng persona vào **mọi workflow mà agent đó dispatch** |
| **Workflow** như `product-brief`, `create-prd` | section `[workflow]` trong `_funcionario/custom/{workflow-name}.toml` | Chỉ áp dụng cho lần chạy của workflow đó |
| **Cấu hình trung tâm** | `[agents.*]`, `[core]`, `[modules.*]` trong `_funcionario/custom/config.toml` | Agent roster và các thiết lập lúc cài đặt cần ghim cho cả tổ chức |
...
- `funcionario-agent-dev.toml`: commit vào git, áp dụng cho cả team
- `funcionario-agent-dev.user.toml`: bị gitignore, dùng cho sở thích cá nhân chồng thêm lên trên
...
- Giữ template trong `{project-root}/docs/` hoặc `{project-root}/_funcionario/custom/templates/` để nó được version cùng với file override
...
**Trường hợp dùng (use case):** Thay đổi *ai đang ngồi trong phòng* cho những skill dựa trên roster như `funcionario-party-mode`, `funcionario-retrospective` và `funcionario-advanced-elicitation`, mà không cần sửa source hay fork. Dưới đây là ba biến thể thường gặp.

### 5a. Rebrand một agent của Funcionário Infinito trên toàn tổ chức
...
Khi bạn yêu cầu party-mode "mời nhóm Star Trek" hoặc "mời phi hành đoàn Enterprise", nó sẽ lọc theo `team = "startrek"` và spawn Spock cùng McCoy dựa trên các descriptor đó. Các agent thật của Funcionário Infinito như Mary hay Amelia vẫn có thể ngồi cùng bàn nếu bạn muốn.
...
Những thiết lập cá nhân như `user_name`, `communication_language` hoặc `user_skill_level` nên vẫn nằm trong `_funcionario/config.user.toml` riêng của từng developer. File chung của team không nên đụng vào các giá trị đó.
...
Tùy chỉnh của Funcionário Infinito chỉ được nạp khi một skill được kích hoạt. Trong khi đó, nhiều công cụ IDE còn nạp một file hướng dẫn toàn cục ở **đầu mọi phiên**, trước cả khi skill nào chạy, như `CLAUDE.md`, `AGENTS.md`, `.cursor/rules/` hay `.github/copilot-instructions.md`. Với những quy tắc phải đúng cả khi bạn đang chat thường, hãy lặp lại phiên bản rút gọn của chúng trong file đó nữa.

**Khi nào nên "đánh đôi"**

- Quy tắc đó đủ quan trọng đến mức một cuộc chat thường, chưa kích hoạt Funcionário Infinito skill nào, cũng vẫn phải tuân theo
...
Chỉ một câu, nhưng được nạp ở mọi phiên. Nó kết hợp với cấu hình `funcionario-agent-dev.toml` để quy tắc có hiệu lực cả trong workflow của Amelia lẫn trong các cuộc trò chuyện ad-hoc với assistant. Mỗi lớp giữ đúng phạm vi của mình:

| Lớp | Phạm vi | Dùng cho |
|---|---|---|
| File hướng dẫn phiên của IDE như `CLAUDE.md` hoặc `AGENTS.md` | Mọi phiên, trước khi bất kỳ skill nào chạy | Quy tắc ngắn, phổ quát, phải sống cả ngoài Funcionário Infinito |
| Tùy chỉnh agent của Funcionário Infinito | Mọi workflow mà agent đó dispatch | Hành vi riêng theo persona/agent |
| Tùy chỉnh workflow của Funcionário Infinito | Một lần chạy workflow | Dạng đầu ra, hook publish, template và logic riêng của workflow |
| Cấu hình trung tâm của Funcionário Infinito | Agent roster và thiết lập cài đặt dùng chung | Ai đang ngồi trong phòng và đường dẫn nào cả team dùng chung |
...
```toml
# _funcionario/custom/funcionario-agent-analyst.toml (cấp agent, Mary sẽ dispatch product-brief)

[agent]
persistent_facts = ["Luôn thêm mục 'Regulatory Review' khi domain liên quan tới healthcare, finance hoặc dữ liệu trẻ em."]
```

Kết quả là Mary nạp quy tắc review pháp lý ngay ở lúc kích hoạt persona. Khi người dùng chọn menu item product-brief, workflow sẽ nạp các convention riêng của nó chồng lên, ghi ra template của doanh nghiệp và publish lên Confluence khi hoàn tất. Mỗi lớp đều đóng góp một phần và không lớp nào đòi hỏi sửa source của Funcionário Infinito.

## Khắc phục sự cố

**Override không có tác dụng?** Hãy kiểm tra file có nằm trong `_funcionario/custom/` và dùng đúng tên thư mục skill không, ví dụ `funcionario-agent-dev.toml`, chứ không phải `funcionario-dev.toml`. Nếu cần, xem lại [Cách tùy chỉnh Funcionário Infinito](./customize-funcionario.md).
