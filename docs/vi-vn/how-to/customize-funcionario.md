---
title: 'Cách tùy chỉnh Funcionário Infinito'
...
- Funcionário Infinito đã được cài trong dự án của bạn (xem [Cách cài đặt Funcionário Infinito](./install-funcionario.md))
- Một cách để chạy resolver script — Funcionário Infinito đang chuẩn hóa sang `uv` (`uv run`, tự cấp Python cho bạn); một `python3` 3.11+ thuần trên PATH vẫn dùng được trong giai đoạn chuyển đổi. Script chỉ dùng stdlib `tomllib`, nên không cần `pip install` gì cả.
...
```text
Ưu tiên 1 (thắng): _funcionario/custom/{skill-name}.user.toml  (cá nhân, bị gitignore)
Ưu tiên 2:         _funcionario/custom/{skill-name}.toml       (team/tổ chức, được commit)
Ưu tiên 3 (gốc):   customize.toml của chính skill       (mặc định)
```

Thư mục `_funcionario/custom/` ban đầu là rỗng. File chỉ xuất hiện khi ai đó thực sự bắt đầu tùy chỉnh.
...
**Quy ước `code` / `id`.** Funcionário Infinito dùng `code` (định danh ngắn như `"BP"` hoặc `"R1"`) và `id` (định danh ổn định dài hơn) làm merge key cho mảng các table. Nếu bạn tự tạo một mảng table muốn có khả năng replace-by-key thay vì append-only, hãy chọn **một** quy ước duy nhất và dùng nhất quán cho toàn bộ mảng. Nếu trộn `code` ở phần tử này và `id` ở phần tử khác, resolver sẽ rơi về chế độ append vì nó không đoán merge theo khóa nào.
...
Tạo thư mục `_funcionario/custom/` ở root dự án nếu nó chưa tồn tại. Sau đó tạo file đặt theo tên skill:

```text
_funcionario/custom/
  funcionario-agent-pm.toml        # override của team (commit vào git)
  funcionario-agent-pm.user.toml   # sở thích cá nhân (gitignore)
```
...
Mọi ví dụ bên dưới đều giả định schema agent phẳng của Funcionário Infinito. Các trường nằm trực tiếp trong `[agent]`, không có các sub-table như `metadata` hay `persona`.
...
activation_steps_append = [
  "Đọc {project-root}/_funcionario/custom/company-glossary.md nếu file tồn tại.",
]
```
...
prompt = """
Đọc {project-root}/_funcionario/custom/compliance-checklist.md
...
**Tham chiếu file.** Khi một trường văn bản cần trỏ tới file (trong `persistent_facts`, `activation_steps_prepend`, `activation_steps_append`, hoặc `prompt` của menu item), hãy dùng đường dẫn đầy đủ dựa trên `{project-root}`. Dù file nằm cạnh override trong `_funcionario/custom/`, bạn vẫn nên viết rõ là `{project-root}/_funcionario/custom/info.md`. Agent sẽ resolve `{project-root}` ở runtime.

### 4. Cá nhân và team

**File của team** (`funcionario-agent-pm.toml`): commit vào git, áp dụng cho cả tổ chức. Dùng cho compliance rules, company persona, năng lực tùy chỉnh dùng chung.

**File cá nhân** (`funcionario-agent-pm.user.toml`): tự động bị gitignore. Dùng cho điều chỉnh giọng điệu, sở thích workflow cá nhân và các fact riêng mà agent cần lưu ý cho riêng bạn.
...
Khi agent được kích hoạt, `SKILL.md` của nó sẽ gọi một shared Python script để merge ba lớp nói trên và trả về block kết quả ở dạng JSON. Script này chỉ dùng `tomllib` của Python stdlib (không có dependency ngoài). Funcionário Infinito đang chuẩn hóa sang `uv run` để chạy các script này (uv tự cấp một bản Python phù hợp cho bạn); một `python3` thuần vẫn dùng được trong giai đoạn chuyển đổi:
...
`--skill` trỏ vào thư mục skill đã cài, nơi có file `customize.toml`. Tên skill được lấy từ basename của thư mục, sau đó script sẽ tự tìm `_funcionario/custom/{skill-name}.toml` và `{skill-name}.user.toml`.
...
uv run {project-root}/_funcionario/scripts/resolve_customization.py \
  --skill /duong-dan/tuyet-doi/toi/funcionario-agent-pm \
  --project-root {project-root} \
  --key agent
...
Workflow, tức các skill điều phối tiến trình nhiều bước như `funcionario-product-brief`, dùng cùng cơ chế override như agent. Khác biệt là bề mặt tùy chỉnh của chúng nằm dưới `[workflow]` thay vì `[agent]`:
...
4. Nạp config (`_funcionario/bmm/config.yaml`) và resolve các biến chuẩn như tên dự án, ngôn ngữ, đường dẫn, ngày tháng
5. Chào người dùng
...
`customize.toml` theo từng skill bao phủ **hành vi sâu** như hook, menu, `persistent_facts`, override persona cho một agent hay workflow đơn lẻ. Một bề mặt khác sẽ bao phủ **trạng thái cắt ngang** như các câu trả lời lúc cài đặt và roster agent mà những skill bên ngoài như `funcionario-party-mode`, `funcionario-retrospective` và `funcionario-advanced-elicitation` sử dụng. Bề mặt đó nằm trong bốn file TOML ở root dự án:

```text
_funcionario/config.toml               (do installer quản lý) team scope:   câu trả lời lúc cài đặt + agent roster
_funcionario/config.user.toml          (do installer quản lý) user scope:   user_name, language, skill level
_funcionario/custom/config.toml        (do con người viết)    team overrides (commit vào git)
_funcionario/custom/config.user.toml   (do con người viết)    personal overrides (gitignore)
```

### Merge bốn lớp

```text
Ưu tiên 1 (thắng): _funcionario/custom/config.user.toml
Ưu tiên 2:         _funcionario/custom/config.toml
Ưu tiên 3:         _funcionario/config.user.toml
Ưu tiên 4 (gốc):   _funcionario/config.toml
```
...
- Các section `[core]` và `[modules.<code>]`: chứa câu trả lời khi cài. `scope = team` sẽ được ghi vào `_funcionario/config.toml`; `scope = user` sẽ nằm trong `_funcionario/config.user.toml`
...
- `_funcionario/config.toml` và `_funcionario/config.user.toml` sẽ **được tạo lại sau mỗi lần cài đặt** từ những câu trả lời mà installer thu thập. Hãy coi chúng là output chỉ đọc; mọi chỉnh sửa trực tiếp sẽ bị ghi đè ở lần cài tiếp theo. Nếu muốn thay đổi bền vững một giá trị cài đặt, hãy chạy lại installer hoặc chồng giá trị đó bằng `_funcionario/custom/config.toml`
- `_funcionario/custom/config.toml` và `_funcionario/custom/config.user.toml` sẽ **không bao giờ** bị installer động vào. Đây mới là bề mặt đúng để thêm custom agent, override descriptor của agent, ép các thiết lập dùng chung cho team và ghim mọi giá trị bạn muốn giữ nguyên bất kể câu trả lời lúc cài là gì

### Ví dụ: đổi thương hiệu cho một agent

```toml
# _funcionario/custom/config.toml (commit vào git, áp dụng cho mọi developer)
...
Resolver sẽ merge đè lên `[agents.funcionario-agent-pm]` do installer sinh ra. `funcionario-party-mode` và mọi roster consumer khác sẽ tự động thấy description mới này.

### Ví dụ: thêm một agent hư cấu

```toml
# _funcionario/custom/config.user.toml (cá nhân, gitignore)
...
|---|---|
| Thêm lời nhắc gọi MCP tool vào mọi dev workflow | Theo từng skill: `_funcionario/custom/funcionario-agent-dev.toml` trong `persistent_facts` |
| Thêm menu item cho một agent | Theo từng skill: `_funcionario/custom/funcionario-agent-{role}.toml` với `[[agent.menu]]` |
| Đổi template đầu ra của một workflow | Theo từng skill: `_funcionario/custom/{workflow}.toml` bằng scalar override |
| Đổi descriptor công khai của một agent | **Cấu hình trung tâm**: `_funcionario/custom/config.toml` ở `[agents.<code>]` |
| Thêm custom agent hoặc agent hư cấu vào roster | **Cấu hình trung tâm**: `_funcionario/custom/config*.toml` với entry mới `[agents.<code>]` |
| Ghim thiết lập cài đặt dùng chung của team | **Cấu hình trung tâm**: `_funcionario/custom/config.toml` trong `[modules.<code>]` hoặc `[core]` |

Trong cùng một dự án, bạn hoàn toàn có thể dùng đồng thời cả hai bề mặt này.

## Ví dụ thực chiến

Để xem các recipe thiên về doanh nghiệp như định hình một agent trên mọi workflow mà nó dispatch, ép workflow tuân thủ convention nội bộ, publish output lên Confluence và Jira, tùy chỉnh agent roster, hoặc thay template đầu ra bằng template riêng của tổ chức, hãy xem [Cách mở rộng Funcionário Infinito cho tổ chức của bạn](./expand-funcionario-for-your-org.md).

## Khắc phục sự cố

**Tùy chỉnh không xuất hiện?**

- Kiểm tra file của bạn có nằm đúng trong `_funcionario/custom/` và dùng đúng tên skill không
...
- Chạy skill `funcionario-customize`. Nó sẽ liệt kê mọi skill có thể tùy chỉnh trong dự án, cho biết skill nào đã có override, rồi dẫn bạn qua quá trình thêm hoặc sửa một override
- Hoặc đọc trực tiếp `customize.toml` của skill. Mọi trường ở đó đều có thể tùy chỉnh, trừ `name` và `title`

**Muốn reset?**

- Xóa file override của bạn trong `_funcionario/custom/`, skill sẽ tự động rơi về cấu hình mặc định tích hợp sẵn
