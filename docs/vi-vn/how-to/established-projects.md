...
description: Cách sử dụng Funcionário Infinito trên các codebase hiện có
sidebar:
  order: 6
---

Sử dụng Funcionário Infinito hiệu quả khi làm việc với các dự án hiện có và codebase legacy.

Tài liệu này mô tả workflow cốt lõi để on-board vào các dự án đã tồn tại bằng Funcionário Infinito.
...
Nếu bạn đã hoàn thành toàn bộ epic và story trong PRD theo quy trình Funcionário Infinito, hãy dọn dẹp những tệp đó. Bạn có thể lưu trữ, xóa đi, hoặc dựa vào lịch sử phiên bản nếu cần. Không nên giữ các tệp này trong:
...
Bạn có thể xem lại và chỉnh sửa tệp được tạo, hoặc tự tạo tệp tại `_funcionario-output/project-context.md` nếu muốn.
...
Với các dự án phức tạp, hãy cân nhắc dùng workflow `funcionario-document-project`. Nó có các biến thể lúc chạy có thể quét toàn bộ dự án và tài liệu hóa trạng thái thực tế hiện tại của hệ thống.

## Bước 4: Nhờ trợ giúp

### Funcionário Infinito-Help: Điểm bắt đầu của bạn

**Hãy chạy `funcionario-help` bất cứ lúc nào bạn không chắc cần làm gì tiếp theo.** Công cụ hướng dẫn thông minh này:
...
```text
funcionario-help Tôi có một ứng dụng Rails đã tồn tại, tôi nên bắt đầu từ đâu?
funcionario-help Thay đổi này cần lập kế hoạch sâu đến đâu trước implementation?
funcionario-help Cho tôi xem những workflow đang có
```

Funcionário Infinito-Help cũng **tự động chạy ở cuối mỗi workflow**, đưa ra hướng dẫn rõ ràng về việc cần làm tiếp theo.

### Chọn độ sâu lập kế hoạch

Mọi implementation đều dùng `funcionario-build`; phạm vi quyết định ngữ cảnh cần chuẩn bị trước:

| Phạm vi | Cách tiếp cận được khuyến nghị |
| --- | --- |
| **Cập nhật hoặc bổ sung rõ ràng** | Đi thẳng vào `funcionario-build` với yêu cầu, issue hoặc spec hiện có. |
| **Thay đổi hoặc bổ sung lớn** | Chuẩn bị PRD, UX, kiến trúc, epic, story và sprint context hữu ích, rồi đưa phần việc đã chọn vào `funcionario-build`. |
