...
The forge cross-examines witnesses: the installed FUNCIONARIO agents, plus any
custom personas and party groups the user has authored for `funcionario-party-mode`.
...
What it returns (JSON, stdout):
  * agents   — the installed FUNCIONARIO roster: the default room, always present.
...
`funcionario-party-mode`'s resolved customization when that skill is found beside
...
PARTY_SKILL = "funcionario-party-mode"
...
def load_agents(project_root: Path):
    """Installed FUNCIONARIO agents as {code: entry}. (dict, resolved_ok).
    ...
    """
    script = project_root / "_funcionario" / "scripts" / "resolve_config.py"
...
def find_party_skill(project_root: Path, skill_root: Path):
    """Locate the installed funcionario-party-mode skill dir, or None.
    ...
    candidates = [
        ...
        project_root / "_funcionario" / "skills" / PARTY_SKILL,
    ]
    for c in candidates:
...
def load_party_workflow(project_root: Path, party_skill: Path):
    """Merged [workflow] table for funcionario-party-mode (base + user overrides)."""
    resolver = project_root / "_funcionario" / "scripts" / "resolve_customization.py"
    data = _run_json(
        [
            sys.executable,
            str(resolver),
...
def load_party_overrides(project_root: Path):
    ...
    """
    custom = project_root / "_funcionario" / "custom"
...
def _alias(code: str) -> str:
    """Short alias for an installed agent code: funcionario-agent-analyst -> analyst."""
    for prefix in ("funcionario-agent-", "funcionario-"):
...
def main():
    ...
    ap.add_argument("--skill", required=True, help="Path to the funcionario-forge-idea skill dir")
    args = ap.parse_args()
