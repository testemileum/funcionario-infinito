---
name: funcionario-correct-course
...
Run: `uv run {project-root}/_funcionario/scripts/resolve_customization.py --skill {skill-root} --project-root {project-root} --key workflow`

**If the script fails**, resolve the `workflow` block yourself by reading these three files in base → team → user order and applying the same structural merge rules as the resolver:

1. `{skill-root}/customize.toml` — defaults
2. `{project-root}/_funcionario/custom/{skill-name}.toml` — team overrides
3. `{project-root}/_funcionario/custom/{skill-name}.user.toml` — personal overrides
...
Run: `uv run {project-root}/_funcionario/scripts/resolve_config.py --project-root {project-root} --key core.project_name --key modules.bmm.implementation_artifacts --key modules.bmm.planning_artifacts --key modules.bmm.project_knowledge`
...
| Project Context | `AGENTS.md` in the affected repo (the `funcionario:context` block) | FULL_LOAD |
...
1. **Read `AGENTS.md`** in the repo the change affects — the block between the `funcionario:context` markers carries the policy, frozen paths, and conventions a course correction must respect.
...
<action>Run: `uv run {project-root}/_funcionario/scripts/resolve_customization.py --skill {skill-root} --project-root {project-root} --key workflow.on_complete` — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
</step>
