---
name: funcionario-product-brief
...
At the opening greeting, let the user know they can invoke `funcionario-party-mode` for multi-agent perspectives or `funcionario-advanced-elicitation` for deeper exploration at any point.
...
5. Resolve config: `uv run {project-root}/_funcionario/scripts/resolve_config.py --project-root {project-root} --key core.project_name --key modules.bmm.planning_artifacts`. `{date}` is the current system datetime.
...
4. Tell the user it is ready: local paths and external destinations (URLs returned from handoffs). Invoke `funcionario-help` to suggest what next steps make sense in the funcionario method ecosystem.
