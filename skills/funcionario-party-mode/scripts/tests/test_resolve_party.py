...
AGENTS = {
    "funcionario-agent-analyst": {"name": "Mary", "icon": "📊", "title": "Analyst"},
    "funcionario-agent-pm": {"name": "John", "icon": "📋", "title": "PM"},
}


class TestAlias(unittest.TestCase):
    def test_strips_known_prefixes(self):
        self.assertEqual(rp._alias("funcionario-agent-analyst"), "analyst")
        self.assertEqual(rp._alias("funcionario-foo"), "foo")
...
class TestBuildCollective(unittest.TestCase):
    def test_installed_agents_indexed_by_code_alias_and_name(self):
        ...
        self.assertEqual(set(col), {"funcionario-agent-analyst", "funcionario-agent-pm"})
        self.assertEqual(idx["analyst"], "funcionario-agent-analyst")  # alias
        self.assertEqual(idx["mary"], "funcionario-agent-analyst")  # name (ci)
        self.assertEqual(idx["funcionario-agent-pm"], "funcionario-agent-pm")  # full code
        self.assertEqual(col["funcionario-agent-analyst"]["source"], "installed")
    ...
    def test_custom_overrides_installed_by_alias(self):
        ...
        self.assertEqual(col["funcionario-agent-analyst"]["source"], "custom")
        self.assertEqual(col["funcionario-agent-analyst"]["name"], "Mary-Custom")
...
class TestResolveMembers(unittest.TestCase):
    ...
    def test_resolves_in_listed_order_and_flags_unknowns(self):
        ...
        self.assertEqual([m["code"] for m in resolved], ["morpheus", "funcionario-agent-analyst"])
...
class TestInstalledCodesIsDefaultRoom(unittest.TestCase):
    ...
    def test_pure_custom_excluded_override_kept_in_default_room(self):
        ...
        self.assertEqual(installed, ["funcionario-agent-analyst", "funcionario-agent-pm"])
        default_room = [col[c]["code"] for c in installed]
        self.assertEqual(default_room, ["funcionario-agent-analyst", "funcionario-agent-pm"])
