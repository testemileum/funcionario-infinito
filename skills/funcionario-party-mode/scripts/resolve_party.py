...
Merges the installed FUNCIONARIO agents with the user's custom `party_members`
...
def load_workflow(project_root: Path, skill_root: Path):
    ...
    script = project_root / "_funcionario" / "scripts" / "resolve_customization.py"
    data = _run_json(
        [
            sys.executable,
            str(script),
...
def main():
    ...
    ap.add_argument("--skill", required=True, help="Path to the funcionario-party-mode skill dir")
