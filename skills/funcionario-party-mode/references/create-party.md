...
A guided authoring flow that turns an idea — a themed cast, a one-off persona, or a pile of raw profile data — into custom party members and groups, written to the user's customize.toml override. The output is configuration; `funcionario-customize` does the actual write.

## What you're producing

Sparse `[workflow]` override entries for `funcionario-party-mode`:
...
## Write via funcionario-customize
...
Compose the sparse override and hand it to `funcionario-customize` to place, confirm, and write — target skill `funcionario-party-mode`, `[workflow]` surface. Default to the **user** override (`funcionario-party-mode.user.toml`); offer the **team** file when the party is meant to be shared. Hand it the exact entries: the `party_members` tables, any `party_groups` table (including its `memory` flag), and `default_party` if the user opted in. Keep it sparse — only the new entries, never a copy of the base customize.toml. `funcionario-customize` shows the TOML, waits for an explicit yes, writes, and verifies the merge; don't write the file yourself.
