...
2. **Otherwise:**
   ...
   - If `{spec_file}` is unknown or missing, create `{{ config.implementation_artifacts }}/funcionario-build-auto-result-<slug-or-timestamp>.md` with:
     ```markdown
     ---
     status: <final status>
     ---

     # Funcionário Infinito Build Auto Result
