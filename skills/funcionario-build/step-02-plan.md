...
Before approving, you can open the spec file in an editor or ask me questions and tell me what to change. You can also use `funcionario-advanced-elicitation` or `funcionario-party-mode`, ideally in another session to avoid context bloat.
...
- **Approve and stop** — approve the spec, leave it `ready-for-dev`, and stop so a fresh `funcionario-build` session can resume at implementation.
