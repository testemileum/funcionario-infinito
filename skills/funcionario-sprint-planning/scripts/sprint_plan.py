...
# split-story keys like 2-6a-...; funcionario-retrospective's sprint_status.py reads
...
# v6 wrote these; they still exist in the wild (v6-shims/funcionario-create-story
...
def cmd_status(args):
    ...
    if by_status.get("review"):
        risks.append(f"{len(by_status['review'])} story(ies) in review — run funcionario-code-review")
    if unrecognized:
    ...
    if by_status.get("in-progress"):
        ...
        if optional_retros:
            recommendation = {
                "skill": "funcionario-retrospective",
                "story_key": None,
