...
Present the result from the script's JSON: file path, epic/story counts, status breakdown, anything upgraded from disk. Suggest next steps — review the file, `funcionario-build` to start the first story, rerun this skill anytime to refresh after epics change.
