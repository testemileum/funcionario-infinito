---
name: funcionario
description: 'Analyzes current state and user query to answer Funcionário Infinito questions or recommend the next skill(s) to use. Use when user asks for help, funcionario help, what to do next, or what to start with in Funcionário Infinito. Also when the user asks to set up, update, or doctor this Funcionário Infinito installation.'
---

# Funcionário Infinito Help

If the user explicitly asks to set up, update, or doctor this Funcionário Infinito
...
Missing Funcionário Infinito project files or scripts never turn an ordinary help request
into setup or doctor.

## Purpose

Orient the user in the Funcionário Infinito skills that are active in their host, answer
...
- If the allowed sources cannot support a general Funcionário Infinito question, fetch the
...
- do not read or fall back to `{project-root}/_funcionario/_config/funcionario-help.csv` or
  any `module-help.csv`;
- do not inspect the legacy installed-module cache as skill discovery state;
- do not require or run `{project-root}/_funcionario/scripts/resolve_config.py`;
...
- do not write files, cache discovery, repair manifests, or create a legacy
  installed-module cache beneath `_funcionario`; and
