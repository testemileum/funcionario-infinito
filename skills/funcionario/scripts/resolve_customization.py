...
def find_project_root(start: Path) -> Path | None:
    """Nearest ancestor holding `_funcionario/`, falling back to the nearest holding `.git`.

    `_funcionario/` outranks `.git` at every depth: a submodule or nested repo carries
    `.git` without being the Funcionário Infinito project, so treating the two as equal stops the
    walk short of the root that owns `_funcionario/custom/`.
    """
    git_root: Path | None = None
    current = start.resolve()
    while True:
        if (current / "_funcionario").is_dir():
            return current
...
def script_project_root() -> Path | None:
    ...
    Skills invoke `{project-root}/_funcionario/scripts/resolve_customization.py`, so when
    this file sits at that path its grandparent is a project root the caller already
    resolved.
    """
    parents = Path(__file__).resolve().parents
    if len(parents) >= 3 and parents[0].name == "scripts" and parents[1].name == "_funcionario":
        return parents[2]
    return None
...
def candidate_project_roots(skill_dir: Path) -> list[Path]:
    ...
    `~`, and any `~/_funcionario` there would otherwise mask the real project's overrides.
    """
    ordered: list[Path] = []
    for root in (
...
def has_override(root: Path, skill_name: str) -> bool:
    custom_dir = root / "_funcionario" / "custom"
...
def main() -> int:
    ...
    parser.add_argument(
        "--project-root",
        "-p",
        help="Explicit project root containing _funcionario/ (recommended)",
    )
    parser.add_argument(
        "--key",
        "-k",
