...
def render(
    ...
    if not (project_root / "_funcionario").is_dir():
        raise RenderError(f"project root does not contain _funcionario/: {project_root}")
    ...
    namespace = project_root / "_funcionario" / "render" / skill_dir.name / f"{slug}-{root_hash}"
