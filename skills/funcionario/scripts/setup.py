...
# ///
"""Inspect installed manifests and materialize or repair project Funcionário Infinito runtime."""
...
# Traces the classic installer leaves under _funcionario. Doctor reports them
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=("Inspect installed Funcionário Infinito manifests or materialize and repair {project-root}/_funcionario.")
    )
    ...
    mode.add_argument(
        "--doctor",
        action="store_true",
        help="repair an existing _funcionario from installed skill payloads",
    )
    ...
    if args.list_config_questions:
        ...
        if args.doctor:
            missing = missing_funcionario_report(project_root)
...
def setup(
    ...
    *,
    ...
    reject_symlinked_funcionario(project_root)
    ...
    if not isinstance(merged, dict):
        raise Exception(f"invalid team config: {project_root / '_funcionario' / 'config.toml'}")
    ...
    for question in pending:
        set_missing_value(
            ...
            project_root / "_funcionario" / "config.toml",
        )
    config_text = render_toml(merged) if pending else base_text
    materialize_funcionario(
        project_root,
        scripts_src,
        config_text,
        modules,
...
def reject_symlinked_funcionario(project_root: Path) -> None:
    funcionario = project_root / "_funcionario"
    if funcionario.is_symlink():
        target = funcionario.resolve()
        raise Exception(
            f"{funcionario} is a symlink to {target}; setup and doctor replace "
            f"_funcionario in place, so run them with --project-root "
            f"{target.parent} to fix the real installation"
        )


def missing_funcionario_report(project_root: Path) -> dict[str, object] | None:
    reject_symlinked_funcionario(project_root)
    funcionario = project_root / "_funcionario"
    if not funcionario.exists():
        return {
            ...
            "message": f"{funcionario} does not exist; run funcionario setup first",
            "changed": False,
        }
    if not funcionario.is_dir():
        raise Exception(f"existing Funcionário Infinito runtime is not a directory: {funcionario}")
    return None
...
def doctor(
    ...
    *,
    ...
    for installed in modules:
        module_root = project_root / "_funcionario" / installed.module
    ...
    shared_changed = not tree_matches(project_root / "_funcionario" / "scripts", shared_tree)
    module_changes = {
        installed.module: not tree_matches(
            project_root / "_funcionario" / installed.module / "scripts",
            declared_scripts_tree(installed.scripts),
        )
        for installed in modules
    }
    funcionario_copy = funcionario_copy_report(skill_root)
    ...
    return {
        ...
        "status": status,
        "changed": changed,
        "funcionario_copy": funcionario_copy,
        ...
        "legacy_leftovers": [
            ...
            if (project_root / "_funcionario").joinpath(*PurePosixPath(relative).parts).exists()
        ],
...
def existing_team_config(project_root: Path) -> tuple[str | None, dict]:
    path = project_root / "_funcionario" / "config.toml"
...
def update_report(project_root: Path, skill_root: Path) -> dict[str, object]:
    ...
    return {
        "mode": "update",
        "funcionario_copy": used_skill_copy_report(skill_root, copies),
...
def read_source_manifest(source: str, copy_item: InstalledCopy) -> bytes:
    if copy_item.parsed.update_source.startswith("file:"):
        ...
        request = urllib.request.Request(
            source,
            headers={"Accept": "text/plain", "User-Agent": "funcionario-update"},
        )
        try:
...
def funcionario_copy_report(skill_root: Path) -> dict[str, object]:
    report = used_skill_copy_report(skill_root, discover_installed_copies(skill_root))
    if report["version"] is not None:
        return report
    raise Exception(f"the funcionario skill copy {skill_root} has no installed {MANIFEST_NAME}")
...
def find_pending_questions(
    ...
    for installed in modules:
        for question in installed.questions:
            ...
            if not has_path(config, path, project_root / "_funcionario" / "config.toml"):
                pending.append(
                    ConfigQuestion(
                        question.module,
                        question.key,
...
def output_folder(config_text: str) -> str:
    folder = tomllib.loads(config_text).get("core", {}).get("output_folder", "_funcionario-output")
    prefix = "{project-root}/"
    if folder.startswith(prefix):
        folder = folder[len(prefix) :]
    return folder or "_funcionario-output"
...
def materialize_doctor(
    ...
    *,
    ...
    funcionario = project_root / "_funcionario"
    staging = Path(tempfile.mkdtemp(prefix="_funcionario.doctor-", dir=project_root))
    try:
        shutil.copytree(
            funcionario,
            staging,
        ...
        replace_dir(staging, funcionario)
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise


def materialize_funcionario(
    project_root: Path,
    scripts_src: Path,
    config_text: str,
    modules: tuple[InstalledModule, ...],
    ...
    staging = Path(tempfile.mkdtemp(prefix="_funcionario.setup-", dir=project_root))
    try:
        ...
        if funcionario.exists():
            ...
            def ignore_scripts_link(directory: str, _names: list[str]) -> set[str]:
                if scripts.is_symlink() and Path(directory) == funcionario:
                    return {"scripts"}
                return set()
...
def replace_dir(src: Path, dest: Path) -> None:
    if not dest.exists():
        src.rename(dest)
        return
    backup = Path(tempfile.mkdtemp(prefix="_funcionario.old-", dir=dest.parent))
    try:
        dest.rename(backup)
    except Exception:
