...
def load_central_config(project_root: Path) -> dict[str, Any]:
    funcionario_dir = project_root / "_funcionario"
    return merge_layers(
        (
            load_toml(funcionario_dir / "config.toml", required=True),
            load_toml(funcionario_dir / "custom" / "config.toml"),
            load_toml(funcionario_dir / "custom" / "config.user.toml"),
        )
    )
...
def load_customization(project_root: Path | None, skill_dir: Path) -> dict[str, Any]:
    ...
    custom_dir = project_root / "_funcionario" / "custom" if project_root else None
    return merge_layers(
        (
