...
class ProjectRootResolutionTests(unittest.TestCase):
    ...
    def test_home_installed_skill_reads_project_override_not_home_funcionario(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            home = Path(temp_dir) / "home"
            project = Path(temp_dir) / "project"
            skill = home / ".claude" / "skills" / "demo-skill"
            write(skill / "customize.toml", facts("shipped default"))
            (home / "_funcionario" / "custom").mkdir(parents=True)
            write(project / "_funcionario" / "custom" / "demo-skill.toml", facts("team override"))
    ...
    def test_walk_prefers_funcionario_over_a_nearer_git_directory(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            project = Path(temp_dir) / "project"
    ...
    def test_notes_when_a_rejected_root_holds_the_only_override(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = home / ".claude" / "skills" / "demo-skill"
            write(skill / "customize.toml", facts("shipped default"))
            write(home / "_funcionario" / "custom" / "demo-skill.toml", facts("home override"))
            (project / "_funcionario" / "custom").mkdir(parents=True)
