...
class ResolveConfigCliTests(unittest.TestCase):
    ...
    def test_full_and_repeated_key_output_follow_layer_precedence(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            custom = root / "_funcionario" / "custom"
            custom.mkdir(parents=True)
            (root / "_funcionario" / "config.toml").write_text('[core]\nname = "base"\nkeep = "yes"\n', encoding="utf-8")
            (root / "_funcionario" / "config.user.toml").write_text(
                '[core]\nname = "base-user"\nstray = "ignored"\n', encoding="utf-8"
            )
            ...
            # _funcionario/config.user.toml is old-installer debris, not a layer:
    ...
    def test_writes_emoji_json_when_stdout_encoding_is_cp1252(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "_funcionario").mkdir(parents=True)
            (root / "_funcionario" / "config.toml").write_text(
                '[agents]\nname = "Analyst"\nicon = "📊"\n',
                encoding="utf-8",
            )
