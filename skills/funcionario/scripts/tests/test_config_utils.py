...
class ConfigUtilsTests(unittest.TestCase):
    ...
    def test_filesystem_layer_precedence(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            funcionario = root / "_funcionario"
            custom = funcionario / "custom"
            skill = funcionario / "bmm" / "sample-skill"
            custom.mkdir(parents=True)
            skill.mkdir(parents=True)
            (funcionario / "config.toml").write_text('[value]\norder = "base-team"\n', encoding="utf-8")
            (funcionario / "config.user.toml").write_text(
                '[value]\norder = "base-user"\nstray = "ignored"\n', encoding="utf-8"
            )
            ...
            # _funcionario/config.user.toml is old-installer debris (setup.py's
