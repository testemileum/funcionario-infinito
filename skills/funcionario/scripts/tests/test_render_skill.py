...
Host skills live outside `_funcionario/`. `_funcionario/` is the project runtime setup
...
SHIPPED_SKILLS = ("funcionario-build-auto", "funcionario-build", "funcionario-code-review")
RENDERED_SKILLS = (*SHIPPED_SKILLS, "funcionario-walkthrough", "funcionario-retrospective")
...
def _namespace_dir(project: Path, skill_name: str) -> Path:
    ...
    return project / "_funcionario" / "render" / skill_name / f"{slug}-{root_hash}"
...
class RenderSkillTests(unittest.TestCase):
    def _workspace(
        ...
        *,
        name: str = "project",
        shared_funcionario: Path | None = None,
        config: str | None = None,
    ) -> SimpleNamespace:
        outer = Path(tempfile.mkdtemp(prefix="funcionario-render-"))
        self.addCleanup(shutil.rmtree, outer, True)
        project = outer / name
        project.mkdir(parents=True)
        (project / "nested" / "cwd").mkdir(parents=True)
        if shared_funcionario is None:
            ...
            (funcionario / "custom").mkdir()
            (funcionario / "config.toml").write_text(
                config if config is not None else _team_config(project),
                encoding="utf-8",
            )
        else:
            (project / "_funcionario").symlink_to(shared_funcionario)
            funcionario = shared_funcionario
        return SimpleNamespace(outer=outer, project=project, funcionario=funcionario)
    ...
    def _cli(
        ...
        return subprocess.run(
            [
                sys.executable,
                str(project / "_funcionario" / "scripts" / "render_skill.py"),
                "--project-root",
                str(project),
                "--skill",
                str(skill),
    ...
    def _assert_rendered(self, workflow: Path, project: Path, skill_name: str) -> Path:
        ...
        artifacts = str(project.resolve() / "_funcionario-output" / "implementation-artifacts")
    ...
    def _assert_halt(self, result: subprocess.CompletedProcess[str], ws: SimpleNamespace) -> None:
        ...
        self.assertFalse((ws.funcionario / "render").exists())

    def test_invocation_precedence_flag_order_and_persistent_isolation(self):
        ws = self._workspace()
        skill = self._fixture_skill(ws, '[workflow]\nmessage = "shipped"\n', "{{ workflow.message }}\n")
        project_file = ws.funcionario / "custom" / "fixture.toml"
        user_file = ws.funcionario / "custom" / "fixture.user.toml"
    ...
    def test_undeclared_persistent_key_halts_and_unread_declared_key_renders(self):
        ws = self._workspace()
        skill = self._fixture_skill(ws, '[workflow]\nmessage = "base"\nunread = "base"\n', "{{ workflow.message }}")
        team = ws.funcionario / "custom" / f"{skill.name}.toml"
        user = ws.funcionario / "custom" / f"{skill.name}.user.toml"
        ...
        generations = set((ws.funcionario / "render" / skill.name).rglob("manifest.json"))
        for layer, content, expected in (
            ...
            with self.subTest(layer=layer.name, content=content):
                ...
                self.assertEqual(set((ws.funcionario / "render" / skill.name).rglob("manifest.json")), generations)
                layer.unlink()
    ...
    def test_skill_root_binds_bundled_scripts_to_the_installed_skill(self):
        ws = self._workspace()
        skill = self._skill(ws, "funcionario-retrospective")
        snap = self._assert_rendered(rs.render(ws.project, skill), ws.project, "funcionario-retrospective")
        markdown = _markdown(snap)
        ...
        elsewhere = _copy_skill(ws.outer / "elsewhere" / "funcionario-retrospective", "funcionario-retrospective")
    ...
    def test_cli_from_nested_cwd_dispatches_one_absolute_workflow(self):
        ws = self._workspace()
        skill = self._skill(ws, "funcionario-build")
        workflow = self._entry(self._cli(ws.project, skill, cwd=ws.project / "nested" / "cwd"))
        self._assert_rendered(workflow, ws.project, "funcionario-build")
        self.assertFalse((ws.funcionario / "scripts" / "__pycache__").exists())
    ...
    def test_identical_input_and_unreferenced_config_reuse_bytes(self):
        ...
        with (ws.funcionario / "config.toml").open("a", encoding="utf-8") as handle:
    ...
    def test_referenced_config_and_source_changes_publish_new_generations(self):
        ws = self._workspace()
        skill = self._skill(ws, "funcionario-build-auto")
        before = rs.render(ws.project, skill)
        before_files = _files(before.parent)
        (ws.funcionario / "custom" / "config.toml").write_text(
    ...
    def test_shared_runtime_keeps_distinct_root_bound_snapshots(self):
        ...
        skill = self._skill(first, "funcionario-build")
        second = self._workspace(name="other", shared_funcionario=first.funcionario)
    ...
    def test_malformed_config_and_customization_halt_without_traceback(self):
        ws = self._workspace()
        skill = self._skill(ws, "funcionario-build")
        (ws.funcionario / "custom" / "config.toml").write_text("[core\nbad", encoding="utf-8")
        result = self._cli(ws.project, skill)
        ...
        (ws.funcionario / "custom" / "config.toml").unlink()
        (ws.funcionario / "custom" / f"{skill.name}.toml").write_text("[workflow\nbad", encoding="utf-8")
        result = self._cli(ws.project, skill)
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_missing_wrong_type_and_non_string_layer_id_halt(self):
        ...
        ws = self._workspace()
        skill = self._skill(ws, "funcionario-build")
        (ws.funcionario / "custom" / f"{skill.name}.toml").write_text(
    ...
    def test_customization_prose_is_not_rescanned_as_source_tokens(self):
        ...
        (ws.funcionario / "custom" / f"{skill.name}.user.toml").write_text(
    ...
    def test_review_layer_override_guard_and_empty_layer_halt(self):
        ...
        (ws.funcionario / "custom" / f"{skill.name}.toml").write_text(disabled, encoding="utf-8")
        review = (rs.render(ws.project, skill).parent / "step-04-review.md").read_text(encoding="utf-8")
    ...
    def test_installed_renderer_identity_change_publishes_a_new_generation(self):
        ...
        with (ws.funcionario / "scripts" / "render_skill.py").open("a", encoding="utf-8") as handle:
    ...
    def test_publication_failure_does_not_dispatch_or_alter_another_root(self):
        ...
        skill = self._skill(stable, "funcionario-build")
        original = rs.render(stable.project, skill)
        original_files = _files(original.parent)
        broken = self._workspace(name="broken", shared_funcionario=stable.funcionario)
