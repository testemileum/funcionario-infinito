...
`uv` must be installed and stop. Do not write `_funcionario` another way.

Run only the flow the user requested. `funcionario update` is inspection only,
`funcionario doctor` repairs an existing runtime, and `funcionario setup` performs setup.
...
include the source-specific reason for a failed check, and state the funcionario copy
...
Doctor requires `{project-root}/_funcionario`. Either doctor command below reports
status `setup-required` when it is absent; relay that instruction to run
`funcionario setup` and stop. Do not create a staging directory or any project output
yourself.
...
staleness, and the funcionario copy/version used. When `legacy_leftovers` is
non-empty, mention that files from a classic Funcionário Infinito installer are present and
...
classic Funcionário Infinito installer left under `_funcionario` are never modified or removed.
It also repairs `_funcionario/scripts` when that path
is a symlink or a copy that is not byte-identical to the packaged `funcionario`
...
the shell) to `{project-root}/.funcionario-help-setup-modules.toml`. If that path
