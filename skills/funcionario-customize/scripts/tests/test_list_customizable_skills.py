...
- a pre-existing team override in _funcionario/custom/
...
class ScannerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.custom = self.root / "_funcionario" / "custom"
    ...
    def test_workflow_only_skill_detected(self):
        _make_skill(
            self.skills,
            "funcionario-create-prd",
            "[workflow]\npersistent_facts = []\n",
            "---\nname: funcionario-create-prd\ndescription: 'Create a PRD.'\n---\n",
        )
        result = MODULE.scan_skills([self.skills], self.root)
    ...
    def test_dual_surface_skill_emits_two_entries(self):
        _make_skill(
            ...
            "funcionario-dual",
            '[agent]\nicon = "x"\n\n[workflow]\npersistent_facts = []\n',
            "---\nname: funcionario-dual\ndescription: Dual.\n---\n",
        )
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]), 1)
        self.assertEqual(len(result["workflows"]), 1)
        self.assertEqual(result["agents"][0]["name"], "funcionario-dual")
        self.assertEqual(result["workflows"][0]["name"], "funcionario-dual")

    def test_skill_without_customize_toml_ignored(self):
        (self.skills / "funcionario-plain").mkdir()
        (self.skills / "funcionario-plain" / "SKILL.md").write_text("# plain\n")
        result = MODULE.scan_skills([self.skills], self.root)
    ...
    def test_existing_team_override_flagged(self):
        ...
        (self.custom / "funcionario-agent-pm.toml").write_text("[agent]\n")
        result = MODULE.scan_skills([self.skills], self.root)
        entry = result["agents"][0]
    ...
    def test_missing_surface_block_reports_error(self):
        _make_skill(self.skills, "funcionario-broken", "[not_a_surface]\nfoo = 1\n")
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]) + len(result["workflows"]), 0)
    ...
    def test_malformed_toml_reports_error_without_aborting(self):
        skill_dir = self.skills / "funcionario-bad"
        skill_dir.mkdir()
        ...
        _make_skill(
            ...
            "funcionario-good",
            '[agent]\nicon = "x"\n',
            "---\nname: funcionario-good\ndescription: Good.\n---\n",
        )
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]), 1)
        self.assertEqual(result["agents"][0]["name"], "funcionario-good")
    ...
    def test_multiple_skills_roots_are_merged(self):
        ...
        _make_skill(
            ...
            '[agent]\nicon = "y"\n',
            "---\nname: funcionario-agent-dev\ndescription: Dev.\n---\n",
        )
        result = MODULE.scan_skills([self.skills, extra_root], self.root)
        names = {a["name"] for a in result["agents"]}
        self.assertEqual(names, {"funcionario-agent-pm", "funcionario-agent-dev"})
