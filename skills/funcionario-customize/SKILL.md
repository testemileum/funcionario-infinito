---
name: funcionario-customize
description: Authors and updates customization overrides for installed Funcionário Infinito skills. Use when the user says 'customize funcionario', 'override a skill', 'change agent behavior', or 'customize a workflow'
---

# Funcionário Infinito Customize

Translate the user's intent into a correctly-placed TOML override file under `{project-root}/_funcionario/custom/` for a customizable agent or workflow skill. Discover, route, author, write, verify.

Scope v1: per-skill `[agent]` overrides (`funcionario-agent-<role>.toml` / `.user.toml`) and per-skill `[workflow]` overrides (`funcionario-<workflow>.toml` / `.user.toml`). Central config (`{project-root}/_funcionario/custom/config.toml`) is out of scope — point users at the [How to Customize Funcionário Infinito guide](https://docs.funcionario-infinito.com.br/how-to/customize-funcionario/).

When the target's `customize.toml` doesn't expose what the user wants, say so plainly. Don't invent fields.

## Preflight

- No `{project-root}/_funcionario/` → Funcionário Infinito isn't installed. Say so, stop.
- `{project-root}/_funcionario/scripts/resolve_customization.py` missing → continue, but Step 6 verify falls back to manual merge.
...
- Every workflow a given agent runs → agent surface (e.g. `funcionario-agent-pm.toml` with `persistent_facts`, `principles`).
- One workflow only → workflow surface (e.g. `funcionario-prd.toml` with `activation_steps_prepend`).
...
Intent outside the exposed surface (step logic, ordering, anything not in `customize.toml`): say so; offer `activation_steps_prepend`/`append` or `persistent_facts` as approximations, or recommend `funcionario-builder` to create a custom skill.
...
**Template swap** (`*_template` scalar): offer to copy the default template to `{project-root}/_funcionario/custom/{skill-name}-{purpose}-template.md`, point the override at the new path, offer to help edit it.

## Step 5: Team or user placement

Under `{project-root}/_funcionario/custom/`:
...
3. Write. Create `{project-root}/_funcionario/custom/` if needed.
4. Verify:
   ```
   uv run {project-root}/_funcionario/scripts/resolve_customization.py --skill <install-path> --project-root {project-root} --key <agent-or-workflow>
   ```
   Show the merged output, point out the changed fields.

   **Resolver missing or fails:** read whichever layers exist — `<install-path>/customize.toml` (base), `{project-root}/_funcionario/custom/{skill-name}.toml` (team), `{project-root}/_funcionario/custom/{skill-name}.user.toml` (user) — apply base → team → user with the same merge rules (scalars override, tables deep-merge, `code`/`id`-keyed arrays merge by key, all other arrays append), describe how the changed fields resolve.
...
- **Central config** (`{project-root}/_funcionario/custom/config.toml`) — see the [How to Customize Funcionário Infinito guide](https://docs.funcionario-infinito.com.br/how-to/customize-funcionario/).
- **Step logic, ordering, behavior not in `customize.toml`** — open a feature request, or use `funcionario-builder` to create a custom skill. Offer to help with either.
