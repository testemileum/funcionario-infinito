...
output_folder = "{project-root}/_funcionario-output"

[modules.bmm]
planning_artifacts = "{project-root}/_funcionario-output/planning-artifacts"
implementation_artifacts = "{project-root}/_funcionario-output/implementation-artifacts"
project_knowledge = "{project-root}/docs"

[agents.funcionario-agent-pm]
module = "bmm"
...
def load_setup():
    ...
    spec = importlib.util.spec_from_file_location("funcionario_setup", SETUP_PY)
...
def write_dest_funcionario(
    ...
    *,
    ...
    funcionario_dir = root / "funcionario"
    write(funcionario_dir / "SKILL.md", "---\nname: funcionario\n---\n")
    dest_setup = funcionario_dir / "scripts" / "setup.py"
    dest_setup.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(SETUP_PY, dest_setup)
    if scripts:
        for name in SHARED_SCRIPTS:
            source = REPO_ROOT / "skills" / "funcionario" / "scripts" / name
            shutil.copy2(source, funcionario_dir / "scripts" / name)
    if assets and config is not None:
        write(funcionario_dir / "assets" / "config.template.toml", config)
    return funcionario_dir


def module_answers_args(project: Path, answers: dict[str, dict[str, str]]) -> list[str]:
    path = project / ".funcionario-help-setup-modules.toml"
    lines: list[str] = []
...
def write_module_skill(
    ...
    *,
    ...
    knowledge: str = "`references/help.md` in the `funcionario` skill",
    version: str = "1.2.3",
...
class FuncionarioSetupTests(unittest.TestCase):
    def test_other_skill_without_setup_is_file_not_found(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            project = Path(temp_dir) / "project"
            project.mkdir()
            missing = project / "_funcionario" / "scripts" / "resolve_config.py"
        ...
        for skill_md in (REPO_ROOT / "skills").rglob("SKILL.md"):
            if skill_md.parent.name == "funcionario":
                continue
    ...
    def test_scripts_present_missing_config_toml_is_hard_error(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = write_dest_funcionario(root)
            project.mkdir()
            scripts = project / "_funcionario" / "scripts"
            ...
            sys.path.insert(0, str(REPO_ROOT / "skills" / "funcionario" / "scripts"))
            try:
            ...
            malformed = project / "_funcionario" / "config.toml"
    ...
    def test_first_setup_fixture_dest_funcionario(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            parsed = tomllib.loads((project / "_funcionario" / "config.toml").read_text(encoding="utf-8"))
            self.assertEqual(
                parsed["agents"]["funcionario-agent-pm"]["name"],
                "John",
            )
    ...
    def test_first_setup_copies_scripts(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self._assert_scripts_identity(project / "_funcionario" / "scripts", skill / "scripts")
    ...
    def test_already_present_paths_are_left_alone(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = write_dest_funcionario(root)
            project.mkdir()
            funcionario = project / "_funcionario"
            write(funcionario / "scripts" / "resolve_config.py", "# old-scripts\n")
            write(funcionario / "config.toml", "# old-config\n")
            write(funcionario / "config.user.toml", "# old-user\n")
            write(funcionario / "core" / "config.yaml", "old: core\n")
            write(funcionario / "bmm" / "config.yaml", "old: bmm\n")
            write(funcionario / "custom" / "keep.txt", "custom-keep\n")
            write(funcionario / "_config" / "funcionario-help.csv", "old-catalog\n")
            output = project / "_funcionario-output"
            write(output / "keep.txt", "output-keep\n")

            result = run_setup(project, skill)
            self.assertEqual(result.returncode, 0, msg=result.stderr)

            self._assert_scripts_identity(funcionario / "scripts", skill / "scripts")
            parsed = tomllib.loads((funcionario / "config.toml").read_text(encoding="utf-8"))
            self.assertEqual(parsed["core"]["project_name"], "proj")
            self._assert_team_tables_match_template(parsed, skill, "proj")
            self.assertEqual(
                (funcionario / "core" / "config.yaml").read_text(encoding="utf-8"),
                "old: core\n",
            )
            self.assertEqual(
                (funcionario / "bmm" / "config.yaml").read_text(encoding="utf-8"),
                "old: bmm\n",
            )
            self.assertEqual(
                (funcionario / "config.user.toml").read_text(encoding="utf-8"),
                "# old-user\n",
            )
            self.assertEqual(
                (funcionario / "custom" / "keep.txt").read_text(encoding="utf-8"),
                "custom-keep\n",
            )
            self.assertEqual(
                (funcionario / "_config" / "funcionario-help.csv").read_text(encoding="utf-8"),
                "old-catalog\n",
            )
            self.assertEqual(
    ...
    def test_already_present_creates_missing_siblings(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = write_dest_funcionario(root)
            project.mkdir()
            write(project / "_funcionario" / "config.toml", "# keep-config\n")
            ...
            self.assertFalse((project / "_funcionario" / "core" / "config.yaml").exists())
            self.assertFalse((project / "_funcionario" / "bmm" / "config.yaml").exists())
            self.assertTrue((project / "_funcionario" / "custom").is_dir())
            self.assertFalse((project / "_funcionario" / "_config" / "funcionario-help.csv").exists())
            self.assertTrue((project / "_funcionario-output").is_dir())
    ...
    def test_setup_leaves_legacy_catalog_alone(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = write_dest_funcionario(root)
            project.mkdir()
            legacy_catalog = project / "_funcionario" / "_config" / "funcionario-help.csv"
    ...
    def test_second_setup_keeps_answers_and_fills_new_keys(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            funcionario = project / "_funcionario"
            team = (funcionario / "config.toml").read_text(encoding="utf-8")
            write(
                funcionario / "config.toml",
                team.replace('project_name = "proj"', 'project_name = "Renamed"'),
            )
            write(funcionario / "custom" / "keep.txt", "custom-keep\n")
            write(funcionario / "config.user.toml", "# keep-user\n")
            write(funcionario / "custom" / "extra.user.toml", "# extra-user\n")
            ...
            self.assertEqual(
                (funcionario / "custom" / "extra.user.toml").read_text(encoding="utf-8"),
                "# extra-user\n",
            )
            self._assert_scripts_identity(funcionario / "scripts", skill / "scripts")
    ...
    def test_broken_or_wrong_scripts_link_is_repaired(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            elsewhere.mkdir()
            os.symlink(elsewhere, funcionario / "scripts", target_is_directory=True)

            result = run_setup(project, skill)
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            self._assert_scripts_identity(funcionario / "scripts", skill / "scripts")

            scripts = funcionario / "scripts"
    ...
    def test_existing_scripts_link_does_not_require_symlink_permission(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            funcionario = project / "_funcionario"
            funcionario.mkdir()
            write(funcionario / "custom" / "keep.txt", "keep\n")
            os.symlink(
                skill / "scripts",
                funcionario / "scripts",
                target_is_directory=True,
    ...
    def test_user_layers_and_leftovers_survive_second_setup(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write(funcionario / "custom" / "keep.txt", "custom-keep\n")
            write(funcionario / "config.user.toml", "# keep-user\n")
            write(funcionario / "custom" / "notes.user.toml", "# custom-user\n")
            write(funcionario / "_config" / "manifest.yaml", "leftover: installer\n")
            write(funcionario / "_config" / "funcionario-help.csv", "old-catalog\n")
            ...
            self.assertEqual(
                (funcionario / "custom" / "notes.user.toml").read_text(encoding="utf-8"),
                "# custom-user\n",
            )
            self.assertEqual(
                (funcionario / "_config" / "manifest.yaml").read_text(encoding="utf-8"),
                "leftover: installer\n",
            )
            self.assertEqual(
                (funcionario / "_config" / "funcionario-help.csv").read_text(encoding="utf-8"),
    ...
    def test_lists_ordered_missing_manifest_questions_without_writing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            write(
                project / "_funcionario" / "config.toml",
                "[modules.alpha]\nexisting = false\n",
            )
            write(project / "_funcionario" / "custom" / "keep.txt", "keep\n")
            write_module_skill(
                root,
                "zeta-skill",
                "zeta",
            ...
            self.assertFalse((project / "_funcionario-output").exists())
            self.assertEqual(list(project.glob("_funcionario.setup-*")), [])
    ...
    def test_manifest_answers_and_nested_scripts_are_installed_and_refreshed(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            original_values = tomllib.loads((funcionario / "config.toml").read_text(encoding="utf-8"))["modules"]["alpha"]
            write(funcionario / "custom" / "keep.txt", "custom\n")
            write(funcionario / "config.user.toml", "# user\n")
            questions = (
                {
                    "key": "existing",
                    "prompt": "Existing",
            ...
            installed = funcionario / "alpha" / "scripts" / "tools" / "check.py"
            self.assertEqual(
                installed.read_bytes(),
                (module_skill / script_path).read_bytes(),
            )
            self.assertFalse((funcionario / "scripts" / "tools" / "check.py").exists())
            self.assertEqual((funcionario / "custom" / "keep.txt").read_bytes(), b"custom\n")
            self.assertEqual((funcionario / "config.user.toml").read_bytes(), b"# user\n")
            ...
            self.assertEqual(second.returncode, 0, msg=second.stderr)
            reparsed = tomllib.loads((funcionario / "config.toml").read_text(encoding="utf-8"))
    ...
    def test_module_scripts_with_same_relative_path_stay_isolated(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            funcionario = project / "_funcionario"
            self.assertEqual((funcionario / "alpha" / relative).read_bytes(), b"# alpha\n")
            self.assertEqual((funcionario / "beta" / relative).read_bytes(), b"# beta\n")
            self.assertFalse((funcionario / relative).exists())
    ...
    def test_existing_scalar_blocks_declared_descendant_without_overwrite(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = write_dest_funcionario(root)
            project.mkdir()
            config = project / "_funcionario" / "config.toml"
    ...
    def test_identical_manifest_copies_dedupe_and_conflicts_are_atomic(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write(project / "_funcionario" / "config.toml", MINIMAL_CONFIG)
            write(project / "_funcionario" / "custom" / "keep.txt", "keep\n")
            before = {
                path.relative_to(project / "_funcionario"): path.read_bytes()
                for path in (project / "_funcionario").rglob("*")
                if path.is_file()
            }
            write_module_skill(
                root,
    ...
    def test_missing_declared_script_and_invalid_answers_are_atomic(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            answer_path = project / ".funcionario-help-setup-modules.toml"
    ...
    def test_invalid_packaged_manifest_is_source_specific_and_atomic(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            funcionario = project / "_funcionario"
            write(funcionario / "config.toml", MINIMAL_CONFIG)
            write(funcionario / "custom" / "keep.txt", "keep\n")
            ...
            before = {path.relative_to(funcionario): path.read_bytes() for path in funcionario.rglob("*") if path.is_file()}
            ...
            self.assertEqual(
                {path.relative_to(funcionario): path.read_bytes() for path in funcionario.rglob("*") if path.is_file()},
                before,
            )
            self.assertEqual(list(project.glob("_funcionario.setup-*")), [])
    ...
    def test_symlinked_declared_script_is_rejected(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse((project / "_funcionario").exists())
            self.assertEqual(list(project.glob("_funcionario.setup-*")), [])

    def test_symlinked_funcionario_is_rejected_before_any_write(self):
        if not symlink_to_temp_dir_succeeds():
            self.skipTest("symlinks not available")
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            project.mkdir()
            skill = write_dest_funcionario(root)
            real = root / "real-install" / "_funcionario"
            write(real / "config.toml", "[core]\nkeep = 1\n")
            os.symlink(real, project / "_funcionario", target_is_directory=True)
            ...
            self.assertTrue((project / "_funcionario").is_symlink())
            self.assertEqual(
                {path.relative_to(real): path.read_bytes() for path in real.rglob("*") if path.is_file()},
                before,
            )
            self.assertEqual(list(project.glob("_funcionario.*-*")), [])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for mode in ("missing", "extra"):
                with self.subTest(mode=mode):
                    ...
                    skill = write_dest_funcionario(case_root)
                    project.mkdir()
                    questions = (
                        {
                            "key": "first",
    ...
    def test_unparseable_team_toml_is_hard_error_and_tree_is_unchanged(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            funcionario = project / "_funcionario"
            write(funcionario / "config.toml", "[broken\n")
            write(funcionario / "core" / "config.yaml", ":::not-yaml\n")
            write(funcionario / "bmm" / "config.yaml", "- nested:\n  - list\n")
            before = {path.relative_to(funcionario): path.read_bytes() for path in funcionario.rglob("*") if path.is_file()}

            result = run_setup(project, skill)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn(str(funcionario / "config.toml"), result.stderr)
    ...
    def test_no_user_toml_without_answers(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_setup(project, skill)
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            self.assertEqual(user_toml_files(project / "_funcionario"), [])
            custom = project / "_funcionario" / "custom"
    ...
    def test_existing_user_toml_is_left_alone(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            skill = write_dest_funcionario(root)
            project.mkdir()
            write(project / "_funcionario" / "config.user.toml", "# keep-user\n")
            result = run_setup(project, skill)
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            self.assertEqual(
                (project / "_funcionario" / "config.user.toml").read_text(encoding="utf-8"),
                "# keep-user\n",
            )

    def test_missing_scripts_does_not_leave_new_funcionario(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            project = root / "proj"
            skill = write_dest_funcionario(root, scripts=False)
            project.mkdir()
            result = run_setup(project, skill)
            self.assertNotEqual(result.returncode, 0)
    ...
    def test_missing_assets_does_not_leave_new_funcionario(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            project = root / "proj"
            skill = write_dest_funcionario(root, assets=False)
            project.mkdir()
            result = run_setup(project, skill)
            self.assertNotEqual(result.returncode, 0)
    ...
    def test_script_copy_failure_preserves_existing_funcionario_and_cleans_staging(
        self,
    ):
        setup = load_setup()
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            funcionario = project / "_funcionario"
            write(funcionario / "scripts" / "resolve_config.py", "# original\n")
            write(funcionario / "custom" / "keep.txt", "keep\n")
            write(funcionario / "config.user.toml", "# user\n")
            (funcionario / "empty-preserved").mkdir()
            before_inode = funcionario.stat().st_ino
            before_files = {path.relative_to(funcionario): path.read_bytes() for path in funcionario.rglob("*") if path.is_file()}
            before_dirs = {path.relative_to(funcionario) for path in funcionario.rglob("*") if path.is_dir()}
            ...
            def fail_second_script_copy(source, dest, *args, **kwargs):
                ...
                if (
                    ...
                    and dest_path.parent.parent.name.startswith("_funcionario.setup-")
                ):
            ...
            self.assertEqual(funcionario.stat().st_ino, before_inode)
            self.assertEqual(
                {path.relative_to(funcionario): path.read_bytes() for path in funcionario.rglob("*") if path.is_file()},
                before_files,
            )
            self.assertEqual(
                {path.relative_to(funcionario) for path in funcionario.rglob("*") if path.is_dir()},
                before_dirs,
            )
            self.assertEqual(list(project.glob("_funcionario.setup-*")), [])
    ...
    def _assert_team_tables_match_template(self, parsed: dict, skill: Path, project_name: str) -> None:
        ...
        self.assertEqual(
            ...
            "{project-root}/_funcionario-output/planning-artifacts",
        )
        self.assertEqual(
            parsed["modules"]["bmm"]["implementation_artifacts"],
            "{project-root}/_funcionario-output/implementation-artifacts",
        )
    ...
    def _assert_first_run_tree(
        ...
        *,
        ...
        self.assertEqual(parsed["core"]["output_folder"], "{project-root}/_funcionario-output")
        ...
        self.assertFalse((funcionario / "core" / "config.yaml").exists())
        self.assertFalse((funcionario / "bmm" / "config.yaml").exists())

        custom = funcionario / "custom"
        self.assertTrue(custom.is_dir())
        self.assertEqual(list(custom.iterdir()), [])
        self.assertEqual(user_toml_files(funcionario), [])

        self.assertFalse((funcionario / "_config" / "funcionario-help.csv").exists())

        self.assertTrue((project / "_funcionario-output").is_dir())


class FuncionarioUpdateDoctorTests(unittest.TestCase):
    def test_update_reports_version_matrix_spreads_and_source_failures_read_only(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_module_skill(project / "sources", "funcionario", "core")
            cases = (
            ...
            self.assertEqual(report["funcionario_copy"]["version"], "1.2.3")
    ...
    def test_doctor_uses_highest_release_adds_only_missing_and_exactly_repairs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            skill = write_dest_funcionario(root)
            write_module_skill(root, "funcionario", "core", version="3.0.0")
            old_question = {
                "key": "existing",
            ...
            write(funcionario / "config.user.toml", "# keep user\n")
            write(funcionario / "custom" / "keep.txt", "keep custom\n")
            write(funcionario / "alpha" / "keep.txt", "keep module\n")
            write(funcionario / "alpha" / "scripts" / "obsolete.py", "obsolete\n")
            write(funcionario / "scripts" / "stale.py", "stale\n")
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            report = json.loads(result.stdout)
            self.assertEqual(report["funcionario_copy"]["skill"], "funcionario")
            self.assertEqual(report["funcionario_copy"]["version"], "3.0.0")
            ...
            config = tomllib.loads((funcionario / "config.toml").read_text(encoding="utf-8"))
            ...
            self.assertEqual(
                (funcionario / "alpha" / "scripts" / "tools" / "new.py").read_bytes(),
                b"new payload\n",
            )
            self.assertFalse((funcionario / "alpha" / "scripts" / "obsolete.py").exists())
            self.assertFalse((funcionario / "alpha" / "scripts" / "old.py").exists())
            self.assertEqual((funcionario / "alpha" / "keep.txt").read_text(), "keep module\n")
            self.assertEqual((funcionario / "custom" / "keep.txt").read_text(), "keep custom\n")
            self.assertEqual((funcionario / "config.user.toml").read_text(), "# keep user\n")
            self.assertTrue(scripts_match(funcionario / "scripts", skill / "scripts"))
            self.assertFalse((funcionario / "scripts").is_symlink())
            self.assertEqual(list(project.glob("_funcionario.doctor-*")), [])
            self.assertEqual(list(project.glob("_funcionario.old-*")), [])
    ...
    def test_doctor_blocks_tied_disagreement_and_missing_runtime_is_actionable(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            skill = write_dest_funcionario(root)
            write_module_skill(root, "funcionario", "core", version="1.0.0")
            write_module_skill(
                root,
                "alpha-one",
                "alpha",
            ...
            self.assertIn("funcionario setup", missing_report["message"])
            self.assertFalse((project / "_funcionario").exists())
            self.assertEqual(list(project.glob("_funcionario.*-*")), [])

            funcionario = project / "_funcionario"
            shutil.copytree(skill / "scripts", funcionario / "scripts")
            (funcionario / "core" / "scripts").mkdir(parents=True)
            write(funcionario / "config.toml", "[modules.alpha]\nkeep = 1\n")
            write(funcionario / "alpha" / "scripts" / "keep.py", "keep\n")
            before = {path.relative_to(funcionario): path.read_bytes() for path in funcionario.rglob("*") if path.is_file()}
    ...
    def test_doctor_invalid_config_and_unreadable_selected_script_are_atomic(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            funcionario = project / "_funcionario"
            write(funcionario / "config.toml", "invalid = [\n")
            write(funcionario / "custom" / "keep.txt", "keep\n")
            before = (funcionario / "custom" / "keep.txt").read_bytes()
            invalid = run_setup_python(project, skill, "--doctor")
            self.assertNotEqual(invalid.returncode, 0)
            self.assertIn(str(funcionario / "config.toml"), invalid.stderr)
            self.assertEqual((funcionario / "custom" / "keep.txt").read_bytes(), before)
            self.assertEqual(list(project.glob("_funcionario.doctor-*")), [])
    ...
    def test_update_reports_current_installation_and_source_disagreement(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for source_root in ("sources-a", "sources-b"):
                write_module_skill(root, "funcionario", "core", update_source=f"file:{source_root}")
                write_module_skill(project / source_root, "funcionario", "core")
            report = json.loads(
    ...
    def test_update_reports_plugin_managed_copies_without_fetching(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            skill = write_dest_funcionario(root)
            write_module_skill(root, "funcionario", "core", update_source="file:sources")
            write_module_skill(project / "sources", "funcionario", "core")
            write_module_skill(root, "plugged", "alpha", update_source="plugin:funcionario-method")
            ...
            self.assertEqual(copy["plugin"], "funcionario-method")
    ...
    def test_doctor_replaces_symlinked_legacy_shared_scripts(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project.mkdir()
            skill = write_dest_funcionario(root)
            write_module_skill(root, "funcionario", "core")
            legacy = root / "legacy-scripts"
            write(legacy / "resolve_config.py", "# legacy shared copy\n")
            funcionario = project / "_funcionario"
            write(funcionario / "config.toml", "[core]\nkeep = true\n")
            (funcionario / "scripts").symlink_to(legacy, target_is_directory=True)
    ...
    def test_first_doctor_after_setup_reports_current(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            funcionario = project / "_funcionario"
            self.assertTrue((funcionario / "core" / "scripts").is_dir())
            self.assertTrue((funcionario / "alpha" / "scripts" / "tool.py").is_file())
    ...
    def test_doctor_reports_legacy_leftovers_read_only(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write(funcionario / "_config" / "funcionario-help.csv", "old-catalog\n")
            write(funcionario / "config.user.toml", "# old-user\n")
            write(funcionario / "core" / "config.yaml", "project_name: proj\n")
            write(funcionario / "bmm" / "config.yaml", "project_name: proj\n")
            write(funcionario / "core" / "v6-shims" / "shim.md", "shim\n")
            ...
            self.assertEqual(
                (funcionario / "core" / "v6-shims" / "shim.md").read_text(encoding="utf-8"),
                "shim\n",
            )
    ...
    def test_doctor_leaves_an_already_correct_runtime_untouched(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            before = {path: path.read_bytes() for path in sorted(funcionario.rglob("*")) if path.is_file()}
            ...
            self.assertTrue(report["current"])
            self.assertEqual(
                {path: path.read_bytes() for path in sorted(funcionario.rglob("*")) if path.is_file()},
                before,
            )
    ...
    def test_doctor_prefers_a_release_over_its_release_candidate(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write(project / "_funcionario" / "config.toml", "[core]\nkeep = true\n")
            ...
            self.assertEqual(
                (project / "_funcionario" / "mixed" / "scripts" / "tool.py").read_bytes(),
                b"release\n",
            )
    ...
    def test_doctor_question_listing_without_a_runtime_is_actionable(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("funcionario setup", report["message"])
            self.assertFalse(report["changed"])
            self.assertFalse((project / "_funcionario").exists())
